...
void KPBackGround::loadStandardBackgroundOasis( KoOasisContext & context, QDomElement * style )
{
    kdDebug()<<"void KPBackGround::loadStandardBackgroundOasis( KoOasisContext & context, QDomElement * style )**********\n";
    QDomElement properties( style->namedItem( "style:properties" ).toElement() );
    if ( !properties.isNull() )
    {
        kdDebug()<<" properties is not null ::::::::::::::\n";
        if ( properties.hasAttribute( "draw:fill" ) )
        {
            kdDebug()<<" fill page \n";
            const QString fill = properties.attribute( "draw:fill" );
            kdDebug()<<" type :"<<fill<<endl;
            if ( fill == "solid" )
            {
                kdDebug()<<"solid \n";
                setBackColor1(QColor(style->attribute( "draw:fill-color" ) ) );

...
bool KPrPieObject::saveOasisObjectAttributes( KPOasisSaveContext &sc ) const
{
    switch( pieType )
    {
        ...
        case PT_ARC:
            ...
            break;
        default:
            kDebug() <<" type of pie not supported";
    }
...
void KPrPieObject::loadOasis(const QDomElement &element, KoOasisContext & context, KPrLoadingInfo *info)
{
    ...
    {
        kDebug()<<" KPrPieObject::loadOasis(const QDomElement &element) type indefined :"<<kind;
        pieType = PT_PIE;
    }
    kDebug()<<" type of pie object :"<<( ( pieType == PT_PIE ) ?"pie" : ( pieType == PT_CHORD )?"cut" :"arc" );
    ...
    kDebug()<<"KPrPieObject::loadOasis(const QDomElement &element) : p_angle :"<<p_angle<<" p_len :"<<p_len;
    if ( pieType == PT_ARC )
    {

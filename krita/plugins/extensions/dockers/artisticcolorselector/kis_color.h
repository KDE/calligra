...
public:
    ...
    inline void setR(float v)
    {
        setRGB(v, core()->rgb(1), core()->rgb(2), core()->hsx(3));
    }
    inline void setG(float v)
    {
        setRGB(core()->rgb(0), v, core()->rgb(2), core()->hsx(3));
    }
    inline void setB(float v)
    {
        setRGB(core()->rgb(0), core()->rgb(1), v, core()->hsx(3));
    }
    inline void setH(float v)
    {
        setHSX(v, core()->hsx(1), core()->hsx(2), core()->hsx(3));
    }
    inline void setS(float v)
    {
        setHSX(core()->hsx(0), v, core()->hsx(2), core()->hsx(3));
    }
    inline void setX(float v)
    {
        setHSX(core()->hsx(0), core()->hsx(1), v, core()->hsx(3));
    }
    inline void setA(float v)
    {
        core()->hsx(3) = qBound(0.0f, v, 1.0f);
    }

    inline QColor         getQColor() const
    {
        return QColor(getR() * 255, getG() * 255, getB() * 255, getA() * 255);
    }
    inline const VecHSXA &getHSX() const
    {
        return core()->hsx;
    }
    inline const VecRGB  &getRGB() const
    {
        return core()->rgb;
    }

    inline void setRGB(float r, float g, float b, float a = 1.0f)
    {
        core()->setRGB(r, g, b, a);
    }
    inline void setHSX(float h, float s, float x, float a = 1.0f)
    {
        core()->setHSX(h, s, x, a);
    }
...
private:
    ...
    inline Core       *core()
    {
        return reinterpret_cast<Core *>(m_coreData + m_offset);
    }
    inline const Core *core() const
    {
        return reinterpret_cast<const Core *>(m_coreData + m_offset);
    }

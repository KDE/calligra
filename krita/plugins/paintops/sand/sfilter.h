...
#ifndef SAND_FILTER_H
#define SAND_FILTER_H

#include <QtGlobal>

/**
 * This class is identical to DynaFilter, in the Dynamic brush.
 * Some modifications where made in its use, but not in the implementation.
 * Perhaps the Simpson's method for integration could better calculate velocities
 * in infinitesimal time intervals (dt approx. 0)
 */
class SandFilter
{
public:
    SandFilter() { initFilterPosition(0.0,0.0);  }
    ~SandFilter() {}

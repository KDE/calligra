...
QString psd_blendmode_to_composite_op(const QString &blendmode)
{
    ...
    // 'idiv' = color burn
    if (blendmode == "idiv") {
        return COMPOSITE_INVERTED_DIVIDE;
    }
    // 'lbrn' = linear burn
    if (blendmode == "lbrn") {
...
QString composite_op_to_psd_blendmode(const QString &compositeop)
{
    ...
    // 'idiv' = color burn
    if (compositeop == COMPOSITE_INVERTED_DIVIDE) {
        return "idiv";
    }
    // 'lbrn' = linear burn
    if (compositeop == COMPOSITE_BURN || compositeop == COMPOSITE_LINEAR_BURN) {
        return "lbrn";
    }
    // 'dkCl' = darker color
    ...
    // 'hMix' = hard mix
    if (compositeop == COMPOSITE_HARD_MIX) {
        return "hMix";
    }
    // 'diff' = difference

...
KisRectangleConstraintWidget::KisRectangleConstraintWidget(QWidget *parent, KisToolRectangleBase *tool) : QWidget(parent)
{
    ...
    connect(chkWidth, SIGNAL(toggled(bool)), this, SLOT(inputsChanged()));
    connect(chkHeight, SIGNAL(toggled(bool)), this, SLOT(inputsChanged()));
    connect(chkRatio, SIGNAL(toggled(bool)), this, SLOT(inputsChanged()));
...
void KisRectangleConstraintWidget::rectangleChanged(const QRectF &rect)
{
    ...
    if (!chkWidth->isChecked()) {
        intWidth->setValue(rect.width());
    }
    if (!chkHeight->isChecked()) {
        intHeight->setValue(rect.height());
    }

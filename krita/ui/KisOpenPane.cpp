...
void KisOpenPane::initRecentDocs()
{
    ...
    connect(recentDocPane, SIGNAL(openUrl(KUrl)), this, SIGNAL(openExistingFile(KUrl)));
    QTreeWidgetItem *item = addPane(header, koIconName("document-open"), recentDocPane, 0);
    connect(recentDocPane, SIGNAL(splitterResized(KisDetailsPane*,QList<int>)),
            this, SIGNAL(splitterResized(KisDetailsPane*,QList<int>)));
    connect(this, SIGNAL(splitterResized(KisDetailsPane*,QList<int>)),
            recentDocPane, SLOT(resizeSplitter(KisDetailsPane*,QList<int>)));
...
void KisOpenPane::initTemplates(const QString &templateType)
{
    ...
    if (!templateType.isEmpty()) {
        ...
        foreach (KisTemplateGroup *group, templateTree.groups()) {
            ...
            connect(pane, SIGNAL(openUrl(KUrl)), this, SIGNAL(openTemplate(KUrl)));

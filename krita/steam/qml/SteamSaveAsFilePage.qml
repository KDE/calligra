         ...
         Rectangle {
            ...
            Item {
                ...
                ListView {
                    ...
                    onCountChanged: {
                        console.debug("Steam file List has: %1 items".arg(count));
                    }

                    function selectText(filename, type) {
                        console.debug("search for text in list: %1.%2".arg(filename).arg(type));
                        model.requestIndexForFilename("%1.%2".arg(filename).arg(type));
                    }

                    function selectItem(index) {
                        if(index>=0) {
                            filesList.selectedFileIndex = index;
                            filesList.selectedFileName = model.get(index).fileName;
                            filesList.selectedFileSizeString = model.get(index).fileSizeMb;
                        } else {
    ...
    Rectangle {
        ...
        Row {
            ...
            PanelTextField {
                ...
                onTextChanged: {
                    filesList.selectText(fileNameField.text, fileType.model.get(fileType.currentIndex).type);
                }
            }

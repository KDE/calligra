 ...
 * main widget.
 *
 * Normally, you will inherit from KoMainWindowBase,
 ...
 * KoMainWindowBase and - unlike the old KoMainWindowBase - may, but do not
 * have to, be deleted by you. KoMainWindowBase will handle that internally.
 *
 ...
 * KoMainWindowBase will take this into account. For fixed size windows set
 * your main widget to a fixed size.
 *
 * Fixed aspect ratios (heightForWidth()) and fixed width widgets are
 * not supported.
 *
 * KoMainWindowBase will set icon, mini icon and caption, which it gets
 ...
 *
 * Note that KoMainWindowBase uses KGlobal::ref() and KGlobal::deref() so that closing
...
class KDEUI_EXPORT KoMainWindowBase : public QMainWindow
{
    friend class KMWSessionManager;
    friend class DockResizeListener;
    KDEUI_DECLARE_PRIVATE(KoMainWindowBase)
    Q_OBJECT
...
public:
     ...
     *
     * Note that a KoMainWindowBase per-default is created with the
     ...
     *
     */
    explicit KoMainWindowBase( QWidget* parent = 0, Qt::WindowFlags f = KDE_DEFAULT_WINDOWFLAGS );
     ...
     *   int n = 1;
     *   while (KoMainWindowBase::canBeRestored(n)){
     ...
     * windows, then you can use KoMainWindowBase::classNameOfToplevel(n)
    ...
    /**
     * List of members of KoMainWindowBase class.
     */
    static QList<KoMainWindowBase*> memberList();
     ...
     * KoMainWindowBase-inherited class constructor, and it will take care
     * of restoring and saving automatically. Make sure you call this
     * _after all_ your *bars have been created.
     *
     * To make sure that KoMainWindowBase propertly obtains the default
...
protected:
    KoMainWindowBase(KoMainWindowBasePrivate &dd, QWidget *parent, Qt::WindowFlags f);

    KoMainWindowBasePrivate * const k_ptr;
private:
 ...
 * widget (each derived from KoMainWindowBase, of course).
 *
 ...
 * call KoMainWindowBase::restore() with the correct arguments. Note that
 ...
 * @see KoMainWindowBase::restore()
 * @see #RESTORE
 * @see KoMainWindowBase::classNameOfToplevel()
 **/
template <typename T>
inline void kRestoreMainWindows() {
  for ( int n = 1 ; KoMainWindowBase::canBeRestored( n ) ; ++n ) {
    const QString className = KoMainWindowBase::classNameOfToplevel( n );

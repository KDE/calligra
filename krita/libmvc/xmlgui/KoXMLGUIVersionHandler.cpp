...
QString KoXmlGuiVersionHandler::findVersionNumber( const QString &xml )
{
...
KoXmlGuiVersionHandler::KoXmlGuiVersionHandler(const QStringList& files)
{
    ...
    if (files.count() == 1) {
        ...
        m_file = files.first();
        m_doc = KoXMLGUIFactory::readConfigFile(m_file);
        return;
    }
    ...
    foreach (const QString &file, files) {
        DocStruct d;
        d.file = file;
        d.data = KoXMLGUIFactory::readConfigFile( file );

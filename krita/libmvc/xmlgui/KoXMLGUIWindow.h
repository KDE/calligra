...
#ifndef KoXMLGUIWINDOW_H
#define KoXMLGUIWINDOW_H
 ...
 *
 * Instead of creating a KoMainWindowBase manually and assigning menus, menu entries,
...
class KDEUI_EXPORT KoXmlGuiWindow : public KoMainWindowBase, public KoXMLGUIBuilder, virtual public KoXMLGUIClient
{
    KDEUI_DECLARE_PRIVATE(KoXmlGuiWindow)
    Q_OBJECT
...
public:
     ...
     * group leader. In that case, the KoMainWindowBase becomes sort of a
     * secondary window.
     *
     ...
     *
     * KoMainWindowBases must be created on the heap with 'new', like:
     * \code
     * KoMainWindowBase *kmw = new KoMainWindowBase(...);
     * kmw->setObjectName(...);
     * \endcode
     *
     * IMPORTANT: For session management and window management to work
     * properly, all main windows in the application should have a
     * different name. If you don't do it, KoMainWindowBase will create
     ...
     * type, append '#' (hash) to the name, and KoMainWindowBase will replace it with numbers to make
     ...
     *
     */
    explicit KoXmlGuiWindow( QWidget* parent = 0, Qt::WindowFlags f = KDE_DEFAULT_WINDOWFLAGS );
    ...
    /**
     * Sets whether KoMainWindowBase should provide a menu that allows showing/hiding
     ...
     * this function) a connection needs to be made to let KoMainWindowBase
     * know when that status (hidden/shown) of the statusbar has changed.
     * For example:
     * connect(action, SIGNAL(activated()),
     *         KoMainWindowBase, SLOT(setSettingsDirty()));
    ...
    // KDE5 TODO: change it to "using KoXMLGUIBuilder::finalizeGUI;"
    virtual void finalizeGUI( KoXMLGUIClient *client );
...
Q_DECLARE_OPERATORS_FOR_FLAGS(KoXmlGuiWindow::StandardWindowOptions)

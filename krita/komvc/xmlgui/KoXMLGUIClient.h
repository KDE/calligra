...
*/
#ifndef KoXMLGUICLIENT_H
#define KoXMLGUICLIENT_H
...
/**
 *
 * A KoXMLGUIClient can be used with KoXMLGUIFactory to create a
 * GUI from actions and an XML document, and can be dynamically merged
 * with other KoXMLGUIClients.
 */
...
public:
  /**
   * Constructs a KoXMLGUIClient which can be used with a
   * KoXMLGUIFactory to create a GUI from actions and an XML document, and
   * which can be dynamically merged with other KoXMLGUIClients.
   */
  KoXMLGUIClient();

  /**
   * Constructs a KoXMLGUIClient which can be used with a KoXMLGUIFactory
   * to create a GUI from actions and an XML document,
   * and which can be dynamically merged with other KoXMLGUIClients.
   *
   ...
   *
   */
  explicit KoXMLGUIClient( KoXMLGUIClient *parent );

  /**
   * Destructs the KoXMLGUIClient.
   *
  ...
  /**
   * This method is called by the KoXMLGUIFactory as soon as the client
   * is added to the KoXMLGUIFactory's GUI.
   */
  void setFactory( KoXMLGUIFactory *factory );
  /**
   * Retrieves a pointer to the KoXMLGUIFactory this client is
   * associated with (will return 0 if the client's GUI has not been built
   * by a KoXMLGUIFactory.
   */
  KoXMLGUIFactory *factory() const;

  /**
   * KoXMLGUIClients can form a simple child/parent object tree. This
   * method returns a pointer to the parent client or 0 if it has no
   * parent client assigned.
   */
  KoXMLGUIClient *parentClient() const;
   ...
   * parent argument.
   */
  void insertChildClient( KoXMLGUIClient *child );

  /**
   * Removes the given @p child from the client's children list.
   */
  void removeChildClient( KoXMLGUIClient *child );

  /**
   * Retrieves a list of all child clients.
   */
  QList<KoXMLGUIClient*> childClients();

  /**
   * A client can have an own KoXMLGUIBuilder.
   * Use this method to assign your builder instance to the client (so that the
   * KoXMLGUIFactory can use it when building the client's GUI)
   *
   * Client specific guibuilders are useful if you want to create
   * custom container widgets for your GUI.
   */
  void setClientBuilder( KoXMLGUIBuilder *builder );

  /**
   * Retrieves the client's GUI builder or 0 if no client specific
   * builder has been assigned via setClientBuilder()
   */
  KoXMLGUIBuilder *clientBuilder() const;
   ...
   *       the client is added to a KoXMLGUIFactory in order to have an
   *       effect.
   *
   * @since 4.4
  */
...
protected:
   ...
   */
  //bool addSuperClient( KoXMLGUIClient * );
   ...
   * to KoXmlGuiWindow::setupGUI() or KoXmlGuiWindow::createGUI().
   *

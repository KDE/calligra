...
{
    return selection()->getOrCreateSelectionPaintDevice(SELECTION);
}
...
void KisSelectionMask::setSelection(KisSelectionSP selection)
{
    if (selection) {
        ...
        this->selection()->createPixelSelection();
        KisFillPainter gc(KisPaintDeviceSP(this->selection()->selectionPaintDevice().data()));

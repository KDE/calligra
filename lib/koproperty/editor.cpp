...
Editor::setList(PtrList *l, bool preservePrevSelection)
{
    ...
    if (d->list) {
        ...
        if (d->currentItem)
            d->list->setPrevSelection( d->currentItem->property()->name() );
        d->list->disconnect(this);
    }
    ...
    if (preservePrevSelection) {
        ...
        if(l)
            selectedPropertyName1 = l->prevSelection();
        //2. in prev. list's current selection
        if(d->list)
            selectedPropertyName2 = d->list->prevSelection();
    }
...
Editor::undo()
{
    ...
    int propertySync = d->currentWidget->property()->isAutoSync();
...
Editor::slotWidgetValueChanged(Widget *widget)
{
    ...
    int propertySync = widget->property()->isAutoSync();
    bool sync = (propertySync != 0 && propertySync != 1) ?
                 d->sync : bool(propertySync);

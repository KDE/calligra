...
void KWFrameLayout::layout( KWDocument* doc, KWFrameSet* mainTextFrameSet,
                            QPtrList<HeaderFooterFrameset>& info,
                            int fromPage, int toPage )
{
    kdDebug() << "KWFrameLayout::layout " << fromPage << " to " << toPage << endl;
    ...
    for ( int pageNum = fromPage ; pageNum <= toPage ; ++pageNum )
    {
        ...
        for ( ; it.current() ; ++it )
        {
            ...
            if ( frameNum != -1 )
            {
                ...
                kdDebug() << "KWFrameLayout::layout page " << pageNum << " adding frame " << frameNum << " from " << fs->name() << endl;
                KoRect rect;
                ...
                kdDebug() << "     rect:" << rect << "   - new top:bottom: " << top << ":" << bottom << endl;

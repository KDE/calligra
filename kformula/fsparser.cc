...
QDomDocument FormulaStringParser::parse()
{
    ...
    //head->output( cout );
    if ( !eol() ) {
        error( i18n( "Aborted parsing at %1:%2" , line ,column ) );
    }
...
ParserNode* FormulaStringParser::parsePrimary()
{
    ...
    case NAME: {
        ...
        if ( currentType == LP ) {
            ...
            expect( RP,  i18n( "'%3' expected at %1:%2" , line ,column,QString( ")" ) ));
    ...
    case LB: {
        ...
        while ( ( currentType != EOL ) && ( currentType != RB ) ) {
            if ( innerBrackets ) {
                expect( LB, i18n( "'%3' expected at %1:%2" ,line , column ,QString( "[" ) ));
            }
            ...
            if ( innerBrackets ) {
                expect( RB, i18n( "'%3' expected at %1:%2" , line , column,QString( "]" ) ));
                if ( currentType == COMMA ) {
                    nextToken();
                }
            }
                ...
                if ( currentType != RB ) {
                    if ( currentType == NEWLINE ) {
                        ...
                        expect( SEMIC,  i18n( "'%3' expected at %1:%2" , line,column ,QString( ";" ) ));
                    }
                }
            }
        ...
        if ( node->columns() == 0 ) {
            error(  i18n( "Null columns in Matrix at %1:%2" , line , column ) );
        }
        if ( node->rows() == 0 ) {
            error( i18n( "Null rows in Matrix at %1:%2" , line , column ) );
        }
        return node;
    }
    case OTHER: {
    ...
    default:
        error( i18n( "Unexpected token at %1:%2" , line , column ) );
    ...
    if ( pos < m_formula.length()-1 ) {
        ...
        if ( ch == '.' ) {
            ...
            if ( ch.isDigit() ) {
                ...
                error( i18n( "A single '.' is not a number at %1:%2", line , column ) );
                return;
            }
        }

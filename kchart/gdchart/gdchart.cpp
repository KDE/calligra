...
#define	NUM_YPOINTS	(sizeof(ypoints) / sizeof(float))
    ...
    for( j=0; j<num_sets; ++j )
		for( i=0; i<num_points; ++i )
			if( GDC_ExtColor ) {
				...
				ExtColor[j][i]            = clrallocate( p, ext_clr );
				if( threeD )
					ExtColorShd[j][i]     = clrshdallocate( p, ext_clr );
			}
    ...
    if( GDC_title ) {
		...
		GDCImageStringNL( im,
						  ...
						  imagewidth/2 - tlen*GDC_fontc[GDC_title_size].w/2,
						  0,
						  GDC_title,
						  titlecolor,
						  GDC_JUSTIFY_CENTER );
    ...
    if( GDC_xtitle ) {
		...
		gdImageString( im,
					   ...
					   imagewidth/2 - strlen(GDC_xtitle)*GDC_fontc[GDC_xtitle_size].w/2,
					   imageheight-GDC_fontc[GDC_xtitle_size].h-1,
					   GDC_xtitle,
					   titlecolor );
    }
    ...
    if( GDC_grid || GDC_yaxis ){	/* grid lines & y label(s) */
		...
		if( do_vol && GDC_ytitle2 ) {
			...
			gdImageStringUp( im,
							 ...
							 imagewidth-(1+GDC_fontc[GDC_ytitle_size].h),
				...
				for( i=1; i<num_points+1; ++i )
					if( uvals[CLOSESET+j*3][i-1] != GDC_NOVALUE ) {
						if( (GDC_HLC_style & GDC_HLC_I_CAP) &&			// bottom half of 'I'
							...
							p->drawPolygon( poly );
							p->setBrush( QBrush( ExtColorShd[LOWSET+j*3][i-1], NoBrush ) );
						...
						if( uvals[HIGHSET+j*3][i-1] != GDC_NOVALUE )
							{											// top 'half'
								...
								p->drawPolygon( poly );
								p->setBrush( QBrush( ExtColorShd[HIGHSET+j*3][i-1], NoBrush ) );
								...
								if( GDC_HLC_style & GDC_HLC_CLOSE_CONNECTED )	/* line from prev close */
									{
										...
										p->drawPolygon( poly );
										p->setBrush( QBrush( ExtColorShd[CLOSESET+j*3][i], NoBrush ) );
						...
						if( GDC_HLC_style & GDC_HLC_DIAMOND )  { // front
							...
							p->drawPolygon( poly );
							p->setBrush( QBrush( ExtColorShd[CLOSESET+j*3][i-1], NoBrush ) );
							p->setPen( ExtColorShd[CLOSESET+j*3][i-1] );
							p->drawPolygon( poly );
							// bottom side

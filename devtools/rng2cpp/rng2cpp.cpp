...
void
isMixed(const RNGItemPtr& item, RNGItems& seen, MixedCheck& mc) {
    if (item->isAttribute() || seen.contains(item)) {
        return;
    }
    seen.insert(item);
    if (item->elementName == "text:list-level-style-number") {
        qDebug() << "text:list-level-style-number" << item->mixedContent;
    }
...
isMixed(const RNGItemPtr& item) {
    ...
    if (item->elementName == "text:list-level-style-number")
        qDebug() << item->cppName << " " << mc.mixed << " " << mc.elementCount;

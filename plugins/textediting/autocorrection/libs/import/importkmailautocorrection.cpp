                ...
                if (loadAttribute == All) {
                    if (xml.readNextStartElement()) {
                        ...
                        if (tagname == QLatin1String("doublequote")) {
                            const auto begin = xml.attributes().value(QStringLiteral("begin")).toString();
                            const auto end = xml.attributes().value(QStringLiteral("end")).toString();
                            if (!begin.isEmpty() && !end.isEmpty()) {
                                mTypographicDoubleQuotes.begin = begin.at(0);
                                mTypographicDoubleQuotes.end = end.at(0);
                            }
                            xml.skipCurrentElement();
                        } else {
                            xml.skipCurrentElement();

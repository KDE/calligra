...
void GrammarCheck::findSentencesInBlock(const QTextBlock &block, QVector<QPair<int,int> > &sentencesInCurrentBlock)
{
    ...
    if (startPos < endPos) {
        ...
        if (isSentenceComplete(block, block.position() + startPos, block.position() + endPos)) {
            sentencesInCurrentBlock.append(qMakePair(startPos, endPos));
            numSentences++;
            if (LOGIC_DEBUG) {
...
bool GrammarCheck::isSentenceComplete(const QTextBlock &block, int startPosition, int endPosition)
{

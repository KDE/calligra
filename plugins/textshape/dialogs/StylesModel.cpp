...
void StylesModel::updateParagraphStyles()
{
    ...
    foreach(KoParagraphStyle *style, styles) {
        if (style != m_styleManager->defaultParagraphStyle()) { //The default paragraph style is not user selectable. It only provides individual property defaults and is not a style per say.

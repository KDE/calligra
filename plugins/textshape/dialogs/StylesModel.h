...
/** This class is used to provide widgets (like the @class StylesCombo) the styles available to the document being worked on. The @class StylesModel can be of
 * two types: character styles or paragraph styles type. This allows the widget to ignore the type of style it is handling. Character styles in ODF can be
 * specified in two ways. First, a named character style, specifying character formatting properties. It is meant to be used on a couple of individual
 * characters. Secondly, a paragraph style also specifies character formatting properties, which are to be considered the default for that particular paragraph.
 * For this reason, the @class Stylesmodel, when of the type @value characterStyle, do not list the paragraph style names. Only the specific named character
 * styles are listed. Additionally, as the first item, a virtual style "As paragraph" is provided. Selecting this "style" will set the character properties as
 * specified by the paragraph style currently applied to the selection. This class requires that a @class KoStyleManager and a @class KoStyleThumbnailer be set.
 * See below methods.
 *
...
public:
    ...
    /** Specify if the combo should provide the virtual style None. This style is a virtual style which equates to no style. It is only relevant for character
       styles. In case the "None" character style is selected, the character formatting properties of the paragraph style are used. A @class StylesModel of the
       @enum Type ParagraphStyle always has this property set to false. On the other hand, the default for a @class StylesModel of the @enum Type CharacterStyle
       is true.
    ...
    */
     ...
     * CAUTION: Populating the style will select the first inserted item. If this model is already set on a view, this might cause the view to emit an item
    ...
    /** Specifies which paragraph style is currently the active one (on the current paragraph). This is used in order to properly preview the "As paragraph"
     * virtual character style. */
    ...
    /** Add a paragraph style to paragraph style list but this style is not applied. */

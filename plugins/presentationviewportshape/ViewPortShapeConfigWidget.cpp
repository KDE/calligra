...
ViewPortShapeConfigWidget::ViewPortShapeConfigWidget()
{
    widget.setupUi(this);
...
void ViewPortShapeConfigWidget::setUnit(KoUnit unit)
{
    widget.cornerRadiusX->setUnit(unit);
    widget.cornerRadiusY->setUnit(unit);
}

void ViewPortShapeConfigWidget::open(KoShape *shape)
{
    m_rectangle = dynamic_cast<PresentationViewPortShape*>(shape);
    if (! m_rectangle)
        return;
...
void ViewPortShapeConfigWidget::save()
{
    if (! m_rectangle)
        return;
...
QUndoCommand * ViewPortShapeConfigWidget::createCommand()
{
    if (! m_rectangle)
        return 0;
    QSizeF size = m_rectangle->size();
    ...
    return new ViewPortShapeConfigCommand(m_rectangle, cornerRadiusX, cornerRadiusY);
}

...
class LcmsColorSpace : public KoColorSpaceAbstract<_CSTraits>, public KoLcmsInfo
{
    ...
    struct Private {
        mutable std::array<quint8, 3> qcolordata{}; // A small BGR buffer for conversion from and to QColor.
...
public:
    ...
    void fromQColor(const QColor &color, quint8 *dst, const KoColorProfile *koprofile = nullptr) const override
    {
        ...
        if (profile == nullptr) {
            ...
            cmsDoTransform(d->defaultTransformations->fromRGB, d->qcolordata.data(), dst, 1);
        } else {
            ...
            cmsDoTransform(d->lastFromRGB, d->qcolordata.data(), dst, 1);
        }
    ...
    void toQColor(const quint8 *src, QColor *c, const KoColorProfile *koprofile = nullptr) const override
    {
        ...
        if (profile == nullptr) {
            ...
            cmsDoTransform(d->defaultTransformations->toRGB, const_cast<quint8 *>(src), d->qcolordata.data(), 1);
        } else {
            ...
            cmsDoTransform(d->lastToRGB, const_cast<quint8 *>(src), d->qcolordata.data(), 1);
        }
    ...
    KoColorTransformation *createBrightnessContrastAdjustment(const quint16 *transferValues) const override
    {
        ...
        adj->cmstransform = cmsCreateMultiprofileTransform(adj->profiles,
                                                           3,
                                                           this->colorSpaceType(),
                                                           this->colorSpaceType(),
    ...
    quint8 difference(const quint8 *src1, const quint8 *src2) const override
    {
        ...
        if (this->opacityU8(src1) == OPACITY_TRANSPARENT_U8 || this->opacityU8(src2) == OPACITY_TRANSPARENT_U8) {

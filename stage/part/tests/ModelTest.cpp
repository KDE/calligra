...
void ModelTest::data()
{
    ...
    QVariant colorVariant = model->data(model->index(0, 0), Qt::BackgroundColor);
...
void ModelTest::rowsAboutToBeInserted(const QModelIndex &parent, int start, int end)
{
    ...
    c.last = model->data(model->index(start - 1, 3, parent));
    c.next = model->data(model->index(start, 3, parent));
    insert.push(c);
}
...
void ModelTest::rowsAboutToBeRemoved(const QModelIndex &parent, int start, int end)
{
    ...
    c.oldSize = model->rowCount(parent);
    c.last = model->data(model->index(start - 1, 3, parent));
    c.next = model->data(model->index(end + 1, 3, parent));
    remove.push(c);
}
...
void ModelTest::rowsRemoved(const QModelIndex & parent, int start, int end)
{
    ...
    Q_ASSERT(c.oldSize - (end - start + 1) == model->rowCount(parent));
    Q_ASSERT(c.last == model->data(model->index(start - 1, 3, c.parent)));
    Q_ASSERT(c.next == model->data(model->index(start, 3, c.parent)));
}

...
bool KWPageStyle::operator==(const KWPageStyle &other) const
{
    ...
    Q_ASSERT_X(!d || (equals == (d->name == other.d->name)), __FUNCTION__, QString("Different styles with the same name are not allowed, equals=%1 name1=%2 name2=%3").arg(equals).arg(d->name).arg(other.d->name).toLocal8Bit());
    return equals;
}

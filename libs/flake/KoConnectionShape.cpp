...
void KoConnectionShape::saveOdf(KoShapeSavingContext & context) const
{
    ...
    if (d->shape1) {
        context.xmlWriter().addAttribute("draw:start-shape", context.xmlid(d->shape1, "shape", true).toString());
    ...
    if (d->shape2) {
        context.xmlWriter().addAttribute("draw:end-shape", context.xmlid(d->shape2, "shape", true).toString());

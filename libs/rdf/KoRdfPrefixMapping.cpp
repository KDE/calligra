...
void KoRdfPrefixMapping::load(QSharedPointer<Soprano::Model> model)
{
    ...
    foreach (const Soprano::Statement &s, listNodes) {
        ...
        QString prefix = KoTextRdfCore::getObject(model, dataBNode, Node::createResourceNode(QUrl(nodePrefix + "prefix"))).toString();
        QString url = KoTextRdfCore::getObject(model, dataBNode, Node::createResourceNode(QUrl(nodePrefix + "url"))).toString();
...
void KoRdfPrefixMapping::save(QSharedPointer<Soprano::Model> model, Soprano::Node context) const
{
    ...
    for (; mi != me; ++mi) {
        ...
        model->addStatement(dataBNode, Node::createResourceNode(QUrl(nodePrefix + "prefix")), Node::createLiteralNode(mi.key()), context);
        model->addStatement(dataBNode, Node::createResourceNode(QUrl(nodePrefix + "url")), Node::createLiteralNode(mi.value()), context);

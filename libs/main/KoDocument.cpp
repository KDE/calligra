...
bool KoDocument::saveFile()
{
    ...
    if (backupFile()) {
        if (url().isLocalFile()) {
            simpleBackupFile(url().toLocalFile(), d->backupPath, QLatin1String("~"));
        }
        else {
            KIO::UDSEntry entry;
            if (KIO::NetAccess::stat(url(),
                ...
                if (d->backupPath.isEmpty())
                    ...
                    backup = KUrl(d->backupPath + '/' + url().fileName());
...
QString KoDocument::autoSaveFile(const QString & path) const
{
    ...
    if (path.isEmpty()) {
        ...
        retval = QString("%1/.%2-%3-%4-autosave%5").arg(QDir::tempPath()).arg(d->parentPart->componentData().componentName()).arg(qApp->applicationPid()).arg(objectName()).arg(extension);
#else
        // On Linux, use a temp file in $HOME then. Mark it with the pid so two instances don't overwrite each other's autosave file
        retval = QString("%1/.%2-%3-%4-autosave%5").arg(QDir::homePath()).arg(d->parentPart->componentData().componentName()).arg(qApp->applicationPid()).arg(objectName()).arg(extension);
#endif
    } else {

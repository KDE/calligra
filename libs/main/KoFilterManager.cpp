...
QString KoFilterManager::importDocument(const QString &url, const QString &documentMimeType, KoFilter::ConversionStatus &status)
{
    ...
    if (m_document) {
        ...
        errorFilter << "You aren't supposed to use import() from a filter!" << Qt::endl;
        status = KoFilter::UsageError;
        return QString();
    }

    if (!chain) {
        errorFilter << "Couldn't create a valid filter chain!" << Qt::endl;
...
KoFilter::ConversionStatus KoFilterManager::exportDocument(const QString &url, QByteArray &mimeType)
{
    ...
    if (!m_graph.isValid()) {
        ...
        if (!d->batch && !userCancelled)
            KMessageBox::error(nullptr, i18n("Could not export file."), i18n("Missing Export Filter"));
...
void KoFilterManager::importErrorHelper(const QString &mimeType, const bool suppressDialog)
{
    ...
    if (!suppressDialog)
        KMessageBox::error(nullptr, tmp, i18n("Missing Import Filter"));
}

...
struct KoColorConversionCacheKey {
    ...
    {
        return (*src == *(rhs.src)) && (*dst == *(rhs.dst)) && (renderingIntent == rhs.renderingIntent) && (conversionFlags == rhs.conversionFlags);
    }
                                                                              ...
                                                                              KoColorConversionTransformation::ConversionFlags _conversionFlags)
{
    ...
    FastPathCacheItem *cacheItem = d->fastStorage.localData();
...
void KoColorConversionCache::colorSpaceIsDestroyed(const KoColorSpace *cs)
{
    ...
    for (QMultiHash<KoColorConversionCacheKey, CachedTransformation *>::iterator it = d->cache.begin(); it != endIt;) {
        if (it.key().src == cs || it.key().dst == cs) {
            Q_ASSERT(it.value()->available()); // That's terribely evil, if that assert fails, that means that someone is using a color transformation with a
                                               // color space which is currently being deleted
            delete it.value();
            it = d->cache.erase(it);
        } else {
            ++it;
...
KoCachedColorConversionTransformation::KoCachedColorConversionTransformation(KoColorConversionCache *cache,
                                                                             KoColorConversionCache::CachedTransformation *transfo)
    : d(new Private)
{

 ...
 * otherwise after
 */
KOWIDGETS_EXPORT void addResourceType(const char *type, const char *basetype, const QString &relativeName, bool priority = true);
 ...
 *         specified filter.
 */
KOWIDGETS_EXPORT QStringList findAllResources(const char *type, const QString &filter = QString(), SearchOptions options = NoSearchOptions);

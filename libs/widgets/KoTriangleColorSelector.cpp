...
{
    d->init();
    connect(displayRenderer, &KoColorDisplayRendererInterface::displayConfigurationChanged, this, &KoTriangleColorSelector::configurationChanged);
}
    ...
    for (int y = 0; y < d->sizeColorSelector; y++) {
        ...
        for (int x = 0; x < d->sizeColorSelector; x++) {
            ...
            if (norm <= d->wheelNormExt + 1.0 && norm >= d->wheelNormInt - 1.0) {
                qreal acoef = 1.0;
                if (norm > d->wheelNormExt)
                    acoef = (1.0 + d->wheelNormExt - norm);
                else if (norm < d->wheelNormInt)
                    acoef = (1.0 - d->wheelNormInt + norm);
...
void KoTriangleColorSelector::selectColorAt(int _x, int _y, bool checkInWheel)
{
    ...
    if (((norm < d->wheelNormExt) && (norm > d->wheelNormInt) && d->handle == NoHandle) || d->handle == HueHandle) {
        d->handle = HueHandle;

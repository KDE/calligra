...
VStrokeDlg::VStrokeDlg( KarbonPart *part ) : QTabDialog ( 0L, 0, true ), m_part( part )
{
	...
	connect( mHue, SIGNAL( valueChanged(int) ), this, SLOT( slotUpdateFromHSVSpinBoxes( int ) ) );
	connect( mSaturation, SIGNAL( valueChanged(int) ), this, SLOT( slotUpdateFromHSVSpinBoxes( int ) ) );
	connect( mValue, SIGNAL( valueChanged(int) ), this, SLOT( slotUpdateFromHSVSpinBoxes( int ) ) );
	...
	if( part->document().selection().count() > 0 ) // there is a selection, so take the opacity of first selected object
		mOpacity->setValue( static_cast<int>(part->document().selection().getFirst()->stroke().color().opacity() * 100.0) );

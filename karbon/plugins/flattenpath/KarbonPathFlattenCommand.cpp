...
public:
    ...
    KoPathPoint * splitSegment( KoPathPoint *p1, KoPathPoint *p2, double splitPosition )
    {
        ...
        KoPathPoint * splitPoint = new KoPathPoint( p1->parent(), p[2], KoPathPoint::CanHaveControlPoint1|KoPathPoint::CanHaveControlPoint2 );
    ...
    : QUndoCommand( parent), d( new Private( path, flatness ) )
{
    ...
    for( uint subpathIndex = 0; subpathIndex < subpathCount; ++subpathIndex )
    {
        ...
        uint pointCount = d->path->pointCountSubpath( subpathIndex );
...
void KarbonPathFlattenCommand::redo()
{
    if( ! d->flattened )
    {
        ...
        for( uint subpathIndex = 0; subpathIndex < subpathCount; ++subpathIndex )
        {
            ...
            uint newPointCount = d->path->pointCountSubpath( subpathIndex );
            // now remove all the control points from the subpath
            for( uint pointIndex = 0; pointIndex < newPointCount; ++pointIndex )
            {

...
# by converting .karbon -> .odg -> .karbon and comparing the initial and final .karbon files.
# We use the karbon format as a "dump" of the Karbon data, to check if everything is correct
# in memory, but the point is of course to ensure that the .odg has all the information.

# To use this script, you need to pass the full path to an existing karbon file as argument.
...
oldextension=karbon
oasisextension=odg
oasismimetype=application/vnd.oasis.opendocument.graphics

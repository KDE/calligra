	...
	, m_part( part )
{
	...
	m_width = new KoUnitDoubleSpinBox( group, 0.0, 1000.0, 0.5, 100.0, KoUnit(KoUnit::Millimeter) );
	m_heightLabel = new QLabel( i18n( "Height:" ), group );
	m_height = new KoUnitDoubleSpinBox( group, 0.0, 1000.0, 0.5, 100.0, KoUnit(KoUnit::Millimeter) );
...
VSinusTool::setup( KActionCollection *collection )
{
	...
	if( m_action == 0 )
	{
		...
		m_action->setDefaultShortcut( KShortcut( Qt::SHIFT+Qt::Key_S ) );

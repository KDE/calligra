...
VSelectionDescription::visitVSelection( VSelection& selection )
{
	if( selection.objects().count() == 1 )
		...
		m_desc = i18np( "One object", "%1 objects", selection.objects().count() );
}
...
{
	m_desc = i18np( "One group, containing one object", "One group, containing %1 objects", group.objects().count() );

...
static inline double ptUnit(double x)
{
    return x * inchtoPtFactor;
}

static inline double odfCornerRadius(qint32 xfigRadius)
{
    return ptUnit(static_cast<double>(xfigRadius) / 80);
}

static inline double odfLineThickness(qint32 xfigLineThickness)
{
    return ptUnit(static_cast<double>(xfigLineThickness) / 80);
}

double XFigOdgWriter::odfLength(qint32 length) const
{
    return ptUnit(static_cast<double>(length) / m_Document->resolution());
}

double XFigOdgWriter::odfLength(double length) const
{
    return ptUnit(length / m_Document->resolution());
}

double XFigOdgWriter::odfXCoord(qint32 x) const
{
    return ptUnit(static_cast<double>(x) / m_Document->resolution());
}

double XFigOdgWriter::odfYCoord(qint32 y) const
{
...
bool XFigOdgWriter::write(XFigDocument *document)
{
    m_Document = document;
...
void XFigOdgWriter::storePixelImageFiles()
{
...
void XFigOdgWriter::storeMetaXml()
{
...
void XFigOdgWriter::storeContentXml()
{
...
void XFigOdgWriter::writeMasterPage()
{
    ...
    if (m_Document->pageSizeType() != XFigPageSizeUnknown) {
        ...
        masterPageLayoutStyle.addProperty(QLatin1String("fo:page-width"), isLandscape ? pageSize.height : pageSize.width);
        masterPageLayoutStyle.addProperty(QLatin1String("fo:page-height"), isLandscape ? pageSize.width : pageSize.height);
    }

    const QString masterPageLayoutStyleName = m_StyleCollector.insert(masterPageLayoutStyle, QLatin1String("masterPageLayoutStyle"));
    ...
    const QString drawingMasterPageStyleName = m_StyleCollector.insert(drawingMasterPageStyle, QLatin1String("drawingMasterPageStyle"));

    masterPageStyle.addAttribute(QLatin1String("draw:style-name"), drawingMasterPageStyleName);

    m_MasterPageStyleName = m_StyleCollector.insert(masterPageStyle, QLatin1String("masterPageStyle"));
}

void XFigOdgWriter::writePage(const XFigPage *page)
{
...
void XFigOdgWriter::writeObject(const XFigAbstractObject *object)
{
...
void XFigOdgWriter::writeCompoundObject(const XFigCompoundObject *groupObject)
{
...
void XFigOdgWriter::writeEllipseObject(const XFigEllipseObject *ellipseObject)
{
    ...
    const QString transformationString = QLatin1String("rotate(") + m_CLocale.toString(ellipseObject->xAxisAngle()) + QLatin1String(")translate(")
        + m_CLocale.toString(odfXCoord(centerPoint.x())) + QLatin1String("pt ") + m_CLocale.toString(odfYCoord(centerPoint.y())) + QLatin1String("pt)");
...
void XFigOdgWriter::writePolylineObject(const XFigPolylineObject *polylineObject)
{
    ...
    const QString polylineStyleName = m_StyleCollector.insert(polylineStyle, QLatin1String("polylineStyle"));
...
void XFigOdgWriter::writePolygonObject(const XFigPolygonObject *polygonObject)
{
    ...
    const QString polygonStyleName = m_StyleCollector.insert(polygonStyle, QLatin1String("polygonStyle"));
...
void XFigOdgWriter::writeBoxObject(const XFigBoxObject *boxObject)
{
...
void XFigOdgWriter::writePictureBoxObject(const XFigPictureBoxObject * /*pictureBoxObject*/)
{
    // TODO
}

void XFigOdgWriter::writeSplineObject(const XFigSplineObject * /*object*/)
{
}
...
void XFigOdgWriter::writeArcObject(const XFigArcObject *arcObject)
{
    ...
    const char *kind = (arcObject->subtype() == XFigArcObject::PieWedgeClosed) ? "section" : (arcObject->fillType() != XFigFillNone) ? "cut" : "arc";
    ...
    writeArrow(arcStyle, arcObject->backwardArrow(), (arcObject->direction() == XFigArcObject::Clockwise) ? LineEnd : LineStart);
    writeArrow(arcStyle, arcObject->forwardArrow(), (arcObject->direction() == XFigArcObject::Clockwise) ? LineStart : LineEnd);
...
void XFigOdgWriter::writeTextObject(const XFigTextObject *textObject)
{
    ...
    const QString transformationString = QLatin1String("rotate(") + m_CLocale.toString(textObject->xAxisAngle()) + QLatin1String(")translate(")
        + m_CLocale.toString(xCoord) + QLatin1String("pt ") + m_CLocale.toString(yCoord) + QLatin1String("pt)");
    m_BodyWriter->addAttribute("draw:transform", transformationString);
    ...
    const QString frameStyleName = m_StyleCollector.insert(frameStyle, QLatin1String("frameStyle"));
    ...
    const QString paragraphStyleName = m_StyleCollector.insert(paragraphStyle, QLatin1String("paragraphStyle"));
    ...
    const QString textSpanStyleName = m_StyleCollector.insert(textSpanStyle, QLatin1String("textSpanStyle"));
...
void XFigOdgWriter::writeZIndex(const XFigAbstractGraphObject *graphObject)
{
    m_BodyWriter->addAttribute("draw:z-index", (1000 - graphObject->depth()));
}

void XFigOdgWriter::writePoints(const QList<XFigPoint> &points)
{
    ...
    const QString viewBoxString = QString::number(minX) + QLatin1Char(' ') + QString::number(minY) + QLatin1Char(' ') + QString::number(width)
        + QLatin1Char(' ') + QString::number(height);
...
void XFigOdgWriter::writeFill(KoGenStyle &odfStyle, const XFigFillable *fillable, qint32 penColorId)
{
    const XFigFillType fillType = fillable->fillType();

    const char *const fillString = (fillType == XFigFillSolid) ? "solid"
...
void XFigOdgWriter::writeDotDash(KoGenStyle &odfStyle, int lineType, double distance)
{
...
void XFigOdgWriter::writeStroke(KoGenStyle &odfStyle, const XFigLineable *lineable)
{
...
void XFigOdgWriter::writeJoinType(KoGenStyle &odfStyle, int joinType)
{
    const char *const linejoin = (joinType == XFigJoinRound) ? "round" : (joinType == XFigJoinBevel) ? "bevel" : "miter";
    odfStyle.addProperty(QLatin1String("draw:stroke-linejoin"), linejoin);
}

void XFigOdgWriter::writeCapType(KoGenStyle &odfStyle, const XFigLineEndable *lineEndable)
{
    const XFigCapType capType = lineEndable->capType();

    const char *const linecap = (capType == XFigCapRound) ? "round" : (capType == XFigCapProjecting) ? "square" : "butt";
    odfStyle.addProperty(QLatin1String("svg:stroke-linecap"), linecap);
}

static const struct ArrowData {
...
} arrowDataList[13] = {
    {
        ...
        "m0 2108v17 17l12 42 30 34 38 21 43 4 29-8 30-21 25-26 13-34 343-1532 339 1520 13 42 29 34 39 21 42 4 42-12 34-30 21-42v-39-12l-4 "
        "4-440-1998-9-42-25-39-38-25-43-8-42 8-38 25-26 39-8 42z" //"m0,1 l0.5,-1 0.5,1"
    },
    {
        // 1
    ...
    },
    {
        ...
        "0 0 1131 1131", //"0 0 1 1",
        "m462 1118-102-29-102-51-93-72-72-93-51-102-29-102-13-105 13-102 29-106 51-102 72-89 93-72 102-50 102-34 106-9 101 9 106 34 98 50 93 72 72 89 51 102 "
        "29 106 13 102-13 105-29 102-51 102-72 93-93 72-98 51-106 29-101 13z", //"m0,0.5 a0.5,0.5 0 1,1 0,0.5z"
    },
    {// 6
     "Half Circle Arrow",
     "0 0 1 1",
...
void XFigOdgWriter::writeArrow(KoGenStyle &odfStyle, const XFigArrowHead *arrow, LineEndType lineEndType)
{
    ...
    const QString arrowStyleName = m_StyleCollector.insert(arrowStyle, QLatin1String("arrowStyle"));

    const char *const marker = (lineEndType == LineStart) ? "draw:marker-start" : "draw:marker-end";
    const char *const markerWidth = (lineEndType == LineStart) ? "draw:marker-start-width" : "draw:marker-end-width";
    const char *const markerCenter = (lineEndType == LineStart) ? "draw:marker-start-center" : "draw:marker-end-center";
...
void XFigOdgWriter::writeHatch(KoGenStyle &odfStyle, int patternType, const QString &colorString)
{
    ...
    const QString hatchStyleName = m_StyleCollector.insert(hatchStyle, QLatin1String("hatchStyle"));

    odfStyle.addProperty("draw:fill-hatch-name", hatchStyleName);
}

void XFigOdgWriter::writeFont(KoGenStyle &odfStyle, const XFigTextObject *textObject)
{
    const XFigFontData &fontData = textObject->fontData();

    odfStyle.addPropertyPt(QLatin1String("fo:font-size"), fontData.mSize);
    const char *const weight = (fontData.mWeight == QFont::Bold) ? "bold" : (fontData.mWeight == QFont::DemiBold) ? "600" : "normal";
    odfStyle.addProperty(QLatin1String("fo:font-weight"), weight);
    const char *const style = (fontData.mStyle == QFont::StyleItalic) ? "italic" : (fontData.mStyle == QFont::StyleOblique) ? "oblique" : "normal";
...
void XFigOdgWriter::writeParagraphStyle(KoGenStyle &odfStyle, const XFigTextObject *textObject)
{
    const XFigTextAlignment textAlignment = textObject->textAlignment();
    const char *const alignmentName = (textAlignment == XFigTextCenterAligned) ? "center"

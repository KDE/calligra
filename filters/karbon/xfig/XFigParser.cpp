...
} arrowHeadTypeMap[arrowHeadTypeMapSize] = {{{XFigArrowHeadStick, XFigArrowHeadStick}},
...
static inline XFigArrowHeadType arrowHeadType(int type3_2, int style3_2)
{
    XFigArrowHeadType result = XFigArrowHeadStick;
    if ((0 <= type3_2) && (type3_2 < arrowHeadTypeMapSize) && ((style3_2 == 0) || (style3_2 == 1))) {
...
static const struct {
    int type3_2;
    XFigLineType type;
} lineTypeMap[] = {{XFig3_2LineDefault, XFigLineDefault},
...
static inline XFigLineType lineType(int type3_2)
{
...
static const struct {
    int type3_2;
    XFigCapType type;
} capTypeMap[] = {{XFig3_2CapButt, XFigCapButt}, {XFig3_2CapRound, XFigCapRound}, {XFig3_2CapProjecting, XFigCapProjecting}};
static const int capTypeMapSize = sizeof(capTypeMap) / sizeof(capTypeMap[0]);

static inline XFigCapType capType(int type3_2)
{
...
static const struct {
    int type3_2;
    XFigJoinType type;
} joinTypeMap[] = {{XFig3_2JoinMiter, XFigJoinMiter}, {XFig3_2JoinRound, XFigJoinRound}, {XFig3_2JoinBevel, XFigJoinBevel}};
static const int joinTypeMapSize = sizeof(joinTypeMap) / sizeof(joinTypeMap[0]);

static inline XFigJoinType joinType(int type3_2)
{
...
static inline XFigFillType fillType(int type3_2)
{
    return ((0 <= type3_2) && (type3_2 <= 40)) ? XFigFillSolid
...
} fillPatternTypeMap[] = {{XFig3_2FillLeftDiagonal30Degree, XFigFillLeftDiagonal30Degree},
...
static inline XFigFillPatternType fillPatternType(int type3_2)
{
...
static const struct XFig3_2PostScriptFontData {
    const char *family;
    QFont::Weight weight;
    QFont::Style style;
...
static const struct PaperSizeIdMap {
    const char *stringId;
    XFigPageSizeType type;
} paperSizeTable[] = {{"Letter", XFigPageSizeLetter},
...
static inline XFigPageSizeType pageSizeType(const QString &stringId)
{
...
XFigDocument *XFigParser::parse(QIODevice *device)
{
    ...
    , m_XFigStreamLineReader(device)
{
    ...
    while (!m_XFigStreamLineReader.readNextObjectLine()) {
        ...
        if (objectCode == XFig3_2ColorObjectId) {
            ...
            XFigAbstractObject *object = (objectCode == XFig3_2EllipseObjectId) ? parseEllipse()
...
bool XFigParser::parseHeader()
{
    ...
    if (!versionString.startsWith(QLatin1String("#FIG 3.")) || (versionString.length() < 8)) {
    ...
    do {
        ...
        if (m_XFigStreamLineReader.readNextLine()) {
            ...
            const XFigPageOrientation pageOrientation = (orientationString == QLatin1String("Landscape")) ? XFigPageLandscape
        ...
        if (m_XFigStreamLineReader.readNextLine()) {
            ...
            const XFigUnitType unitType = (unitTypeString == QLatin1String("Metric")) ? XFigUnitMetric
        ...
        if (m_XFigStreamLineReader.readNextLine(XFigStreamLineReader::CollectComments)) {
            ...
            const XFigCoordSystemOriginType coordSystemOriginType = (coordinateSystemType == 1) ? XFigCoordSystemOriginLowerLeft
...
static inline int parseTwoDigitHexValue(QTextStream &textStream)
{
    int result = 0;
    ...
    int faktor = 1;
    for (int i = 0; i < 2; i++) {
        const int value = ('0' <= digit[i] && digit[i] <= '9') ? digit[i] - '0'
...
XFigAbstractObject *XFigParser::parseArc()
{
    // qDebug()<<"arc";
    ...
    QString line = m_XFigStreamLineReader.line();
    QTextStream textStream(&line, QIODevice::ReadOnly);
    textStream >> sub_type >> line_style >> thickness >> pen_color >> fill_color >> depth >> pen_style >> area_fill >> style_val >> cap_style >> direction
    ...
    const XFigArcObject::Direction arcDirection = (direction == 1) ? XFigArcObject::CounterClockwise :
...
XFigAbstractObject *XFigParser::parseEllipse()
{
    ...
    QString line = m_XFigStreamLineReader.line();
    QTextStream textStream(&line, QIODevice::ReadOnly);
    textStream >> sub_type >> line_style >> thickness >> pen_color >> fill_color >> depth >> pen_style >> area_fill >> style_val >> direction >> angle
        >> center_x >> center_y >> radius_x >> radius_y >> start_x >> start_y >> end_x >> end_y;

    const XFigEllipseObject::Subtype subtype = (sub_type == 1) ? XFigEllipseObject::EllipseByRadii
...
XFigAbstractObject *XFigParser::parsePolyline()
{
    ...
    QString line = m_XFigStreamLineReader.line();
    QTextStream textStream(&line, QIODevice::ReadOnly);
    textStream >> sub_type >> line_style >> thickness >> pen_color >> fill_color >> depth >> pen_style >> area_fill >> style_val >> join_style >> cap_style
...
XFigAbstractObject *XFigParser::parseSpline()
{
    ...
    qint32 sub_type, line_style, thickness, pen_color, fill_color, depth, pen_style, area_fill, cap_style, forward_arrow, backward_arrow, npoints;
    float style_val;

    // this should be a spline
    QString line = m_XFigStreamLineReader.line();
    QTextStream textStream(&line, QIODevice::ReadOnly);
    textStream >> sub_type >> line_style >> thickness >> pen_color >> fill_color >> depth >> pen_style >> area_fill >> style_val >> cap_style >> forward_arrow
...
XFigAbstractObject *XFigParser::parseText()
{
    ...
    QString line = m_XFigStreamLineReader.line();
    QTextStream textStream(&line, QIODevice::ReadOnly);
    textStream >> sub_type >> color >> depth >> pen_style >> font >> font_size >> angle >> font_flags >> height >> length >> x >> y;

    const XFigTextAlignment textAlignment = (sub_type == XFig3_2TextCenterAligned) ? XFigTextCenterAligned
    ...
    for (int i = 0; i < textData.length(); ++i) {
        ...
        if (textChar == QLatin1Char('\\')) {
            ...
            if (isOctalEncodedChar) {
                unsigned char charValue = digitValues[0] * 64 + digitValues[1] * 8 + digitValues[2];
...
XFigAbstractObject *XFigParser::parseCompoundObject()
{
...

    {
        ...
        const XFigBoundingBox boundingBox(XFigPoint(upperLeftX, upperLeftY), XFigPoint(lowerRightX, lowerRightY));
...
XFigArrowHead *XFigParser::parseArrowHead()
{
    ...
    textStream >> arrow_type >> arrow_style >> arrow_thickness >> arrow_width >> arrow_height;

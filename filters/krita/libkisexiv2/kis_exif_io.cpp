...
KisMetaData::Value deviceSettingDescriptionExifToKMD(const Exiv2::Value::AutoPtr value)
{
    ...
    for(int i = 0; i < columns * rows; i++)
    {
        ...
        index = lastIndex + 2;
        kDebug(41008) << "Setting [" << i << "] = " << setting << endl;
...
bool KisExifIO::saveTo(KisMetaData::Store* store, QIODevice* ioDevice) const
{
        ...
        it != store->end(); ++it )
    {
        ...
        if(exivKey.isEmpty())
        {
            kDebug(41008) << entry.qualifiedName() << " is unsavable to EXIF" << endl;
        } else {
            ...
            {
//                 kDebug(41008) << "Saving key " << exivKey << " of KMD value " << entry.value() << endl;
                exifData.add(exifKey, v );
            } else {
                kDebug(41008) << "No exif value was created for " << entry.qualifiedName() << " as " << exivKey << " of KMD value " << entry.value() << endl;
            }
        }
    }
...
bool KisExifIO::loadFrom(KisMetaData::Store* store, QIODevice* ioDevice) const
{
    ...
    kDebug(41008) << "There are " << exifData.count() << " entries in the exif section" << endl;
    ...
    for(Exiv2::ExifMetadata::const_iterator it = exifData.begin();
        it != exifData.end(); ++it)
    {
        kDebug(41008) << "Reading info for key " << it->key().c_str() << endl;
        ...
        {
            kDebug(41008) << it->key().c_str() << " is ignored" << endl;
        ...
        { // load as dc:rights
            ...
            kDebug(41008) << "Ignoring thumbnail tag : " << it->key().c_str() << endl;
        } else {
            kDebug(41008) << "Unknown exif tag, can't load: " << it->key().c_str() << endl;
        }
    }
    ioDevice->close();
    return true;

...
// Note from David Faure: of course not ! Just needs to be reimplemented,
// that's all :)
...
class           TabularData {
  public:
    ...
    int             type;       // alignment of data - 0 = left, 1 =
                                // center, 2 = right, 3 = decimal
};                              // end TabularD
...
class           BorderStyle {
  public:
    ...
    int             green;
    int             blue;
    int             style;      // 0 = solid line, 1 = dash, 2 = dots, 3 = 
                                // pattern, 4 = pattern
  ...
  public:
    TextFormatting() {
        ...
        green = 0;
                     ...
                     int r,
                     int b,
                     int g, int pn, Time t, Date d, int ty):formatId(id),
                    pos(p),
                    len(l),
                    fontSize(s),
                    fontWeight(w),
...
// container for picturn anchor tag name and position
{
  public:
    PictureAnchor() {
    } PictureAnchor (int p, QString k):pos(p),
                    key(k) {
    }
...
// container for table anchor tag name and position
{
  public:
    TableAnchor() {
        pos = -1;
        name = "";
    } TableAnchor   (int p, QString n):pos(p),
                    name(n) {
    }
...
// container for paragraph type, flow and counter information
{
    ...
    ParaLayout() {
        ...
        idRight = -1;
        lineSpacing = -1;
    } int           type;       // indicator for enum type - numeric,
                                // alpha, roman num, etc.
...
// Container for the three classes
{
  public:
    FormatData() {
    } FormatData    (const TextFormatting & t):id(1),
                    text(t) {
    }
    FormatData(const PictureAnchor & p):id(2),
                    pictureAnchor(p) {
    }
    FormatData(const TableAnchor & t):id(6),
                    tableAnchor(t) {
    }
...
class           TableCell {
  public:
    TableCell() {
                     ...
                     const BorderStyle & b, const Frame & f):col(c),
                    row(r),
                    text(t),
                    left(l),
                    right(ri),
...
class           Table {
  public:
    ...
    void            addCell(int c,
                            ...
                            BorderStyle l,
                            BorderStyle ri,
                            BorderStyle tp, BorderStyle b, Frame f);
...
class           Picture {
  public:
    Picture() {
    } Picture       (QString n, QString f):name(n), koStoreName(f) {
    }
...
class           AnchoredInsert {
  public:
    AnchoredInsert() {
    } AnchoredInsert(const Picture & i, int p):type(2),
                    picture(i),
                    pos(p) {
    }
    AnchoredInsert(const Table & i, int p):type(6),
                    table(i),
                    pos(p) {
    }
...
void            ProcessParagraph(QString & paraText,
                                 ...
                                 QString & outputText, ParaLayout & layout,
                                 DocData * docData);
...
void            ProcessPaperTag(QDomNode myNode, void *tagData, QString &);
...
void            ProcessParagraphTag(QDomNode myNode, void *tagData, QString & outputText);      // part
                                                                                                // virtual

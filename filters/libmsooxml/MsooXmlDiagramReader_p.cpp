...
// #define DEBUG_DUMP debugMsooXml << QString("%1Dgm::%2::%3").arg(QString(' ').repeated(level)).arg(typeid(this).name()).arg(__FUNCTION__) << this << "atom="
// << m_tagName
...
ValueCache::ValueCache()
    : m_rect(QRectF(0.0f, 0.0f, 100.0f, 100.0f))
    , m_unmodified(true)
...
void AbstractNode::readAll(Context *context, MsooXmlDiagramReader *reader)
{
    while (!reader->atEnd()) {
        ...
        if (tokenType == QXmlStreamReader::Invalid || tokenType == QXmlStreamReader::EndDocument)
            break;
        if (!reader->isStartElement() && reader->qualifiedName() == m_tagName)
            break;
...
void PointNode::readTextBody(Context *, MsooXmlDiagramReader *reader)
{
    ...
    while (!reader->atEnd()) {
        reader->readNext();
        if (reader->isEndElement() && reader->qualifiedName() == QLatin1String("dgm:t"))
            break;
        switch (s) {
        case Start:
            if (reader->isStartElement() && reader->qualifiedName() == QLatin1String("a:p"))
                s = Paragraph;
            break;
        case Paragraph:
        ...
        case TextRun:
            if (reader->qualifiedName() == QLatin1String("a:t")) {
                if (reader->isStartElement()) {
                    if (!m_text.isEmpty())
                        m_text += ' '; // concat multiple strings into one result
...
void MSOOXML::Diagram::PointNode::dump(QTextStream &device)
{
    foreach (AbstractNode *node, peers()) {
        ...
        PointNode *pointNode = dynamic_cast<PointNode *>(node);
        if (connNode)
            device << "\"" << m_tagName << m_modelId << "\" -> \"" << connNode->m_tagName << connNode->m_modelId << "\"[label=\""
                   << /*m_tagName << m_modelId << " " <<*/ m_text << "\"]\n";
        else if (pointNode)
            device << "\"" << m_tagName << m_modelId << "\" -> \"" << pointNode->m_tagName << pointNode->m_modelId << "\"[label=\""
                   << /*m_tagName << m_modelId << " " <<*/ m_text << "\"]\n";
    }
    foreach (AbstractNode *node, children()) {
        ConnectionNode *connNode = dynamic_cast<ConnectionNode *>(node);
...
AbstractAtom::AbstractAtom(const QString &tagName)
    : QSharedData()
    , m_tagName(tagName)
{
}
    ...
    if (reader->isStartElement()) {
        ...
        if (reader->qualifiedName() == QLatin1String("dgm:layoutNode")) {
            ...
            while (!reader->atEnd()) {
                ...
                if (tokenType == QXmlStreamReader::Invalid || tokenType == QXmlStreamReader::EndDocument)
                    break;
                if (!reader->isStartElement() && reader->qualifiedName() == QLatin1StringView("dgm:varLst"))
                    break;
                if (reader->isStartElement()) {
                    const QXmlStreamAttributes attrs(reader->attributes());
...
QList<AbstractNode *> AbstractAtom::fetchAxis(Context *context,
                                              const QString &_axis,
                                              const QString &_ptType,
                                              const QString &_start,
                                              const QString &_count,
                                              const QString &_step) const
{
...
void AlgorithmAtom::readAll(Context *context, MsooXmlDiagramReader *reader)
{
    ...
    if (type == QLatin1String("composite"))
        m_type = CompositeAlg;
    else if (type == QLatin1String("conn"))
        m_type = ConnectorAlg;
    else if (type == QLatin1String("cycle"))
        m_type = CycleAlg;
    else if (type == QLatin1String("hierChild"))
        m_type = HierChildAlg;
    else if (type == QLatin1String("hierRoot"))
        m_type = HierRootAlg;
    else if (type == QLatin1String("lin"))
        m_type = LinearAlg;
    else if (type == QLatin1String("pyra"))
        m_type = PyramidAlg;
    else if (type == QLatin1String("snake"))
        m_type = SnakeAlg;
    else if (type == QLatin1String("sp"))
        m_type = SpaceAlg;
    else if (type == QLatin1String("tx"))
        m_type = TextAlg;
    else
        m_type = UnknownAlg;
...
void LayoutNodeAtom::finishBuild(Context *context)
{
    ...
    case AlgorithmAtom::CompositeAlg:
        m_algorithmImpl = new CompositeAlgorithm;
        break;
    case AlgorithmAtom::ConnectorAlg:
        m_algorithmImpl = new ConnectorAlgorithm;
        break;
    case AlgorithmAtom::CycleAlg:
        m_algorithmImpl = new CycleAlgorithm;
        break;
    case AlgorithmAtom::HierChildAlg:
        m_algorithmImpl = new HierarchyAlgorithm(false);
        break;
    case AlgorithmAtom::HierRootAlg:
        m_algorithmImpl = new HierarchyAlgorithm(true);
        break;
    case AlgorithmAtom::LinearAlg:
        m_algorithmImpl = new LinearAlgorithm;
        break;
    case AlgorithmAtom::PyramidAlg:
        m_algorithmImpl = new LinearAlgorithm;
        break;
    case AlgorithmAtom::SnakeAlg:
        m_algorithmImpl = new SnakeAlgorithm;
        break;
    case AlgorithmAtom::SpaceAlg:
        m_algorithmImpl = new SpaceAlg;
        break;
    case AlgorithmAtom::TextAlg:
        m_algorithmImpl = new TextAlgorithm;
        break;
    }
}
...
{
    if (needsRelayout == m_needsRelayout)
        return;
...
QMap<QString, QString> LayoutNodeAtom::variables() const
{
    return m_variables;
}
void LayoutNodeAtom::setVariable(const QString &name, const QString &value)
{
...
QVector<QExplicitlySharedDataPointer<LayoutNodeAtom>>
LayoutNodeAtom::fetchLayouts(Context *context, const QString &forAxis, const QString &forName, const QString &ptType) const
{
...
QString ConstraintAtom::dump() const
{
    QString s;
    if (!m_fact.isEmpty())
        s += QString("fact=%1 ").arg(m_fact);
    if (!m_for.isEmpty())
        s += QString("for=%1 ").arg(m_for);
    if (!m_forName.isEmpty())
        s += QString("forName=%1 ").arg(m_forName);
    if (!m_op.isEmpty())
        s += QString("op=%1 ").arg(m_op);
    if (!m_ptType.isEmpty())
        s += QString("ptType=%1 ").arg(m_ptType);
    if (!m_refPtType.isEmpty())
        s += QString("refPtType=%1 ").arg(m_refPtType);
    if (!m_refType.isEmpty())
        s += QString("refType=%1 ").arg(m_refType);
    if (!m_refFor.isEmpty())
        s += QString("refFor=%1 ").arg(m_refFor);
    if (!m_refForName.isEmpty())
        s += QString("refForName=%1 ").arg(m_refForName);
    if (!m_type.isEmpty())
        s += QString("type=%1 ").arg(m_type);
    if (!m_value.isEmpty())
        s += QString("val=%1 ").arg(m_value);
    return s.trimmed();
}
...
void ShapeAtom::writeAtom(Context *context, KoXmlWriter *xmlWriter, KoGenStyles *styles)
{
    ...
    */
    ...
    if (m_type == QLatin1String("ellipse")) {
        ...
        xmlWriter->startElement("draw:enhanced-geometry");
        xmlWriter->addAttribute("draw:enhanced-path",
                                "M 0 414114 C 0 304284 43630 198953 121292 121291 198954 43630 304285 0 414115 0 523945 0 629276 43630 706938 121292 784599 "
                                "198954 828229 304285 828229 414115 828229 523945 784599 629276 706938 706938 629277 784599 523945 828229 414115 828229 304285 "
                                "828229 198954 784599 121292 706938 43631 629276 1 523945 1 414115 L 0 414114 Z N");
        ...
        xmlWriter->startElement("draw:enhanced-geometry");
        xmlWriter->addAttribute("draw:enhanced-path",
                                "M 0 97707 C 0 71793 10294 46941 28618 28618 46942 10294 71794 0 97707 0 L 804191 0 C 830105 0 854957 10294 873280 28618 "
                                "891604 46942 901898 71794 901898 97707 L 901898 488526 C 901898 514440 891604 539292 873280 557615 854956 575939 830104 "
                                "586233 804191 586233 L 97707 586233 C 71793 586233 46941 575939 28618 557615 10294 539291 0 514439 0 488526 L 0 97707 Z N");
        xmlWriter->addAttribute("draw:glue-point-leaving-directions", "-90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90");
        xmlWriter->addAttribute(
            "draw:glue-points",
            "?f20 ?f21 ?f22 ?f23 ?f24 ?f25 ?f26 ?f25 ?f27 ?f23 ?f28 ?f21 ?f28 ?f29 ?f27 ?f30 ?f26 ?f31 ?f24 ?f31 ?f22 ?f30 ?f20 ?f29 ?f20 ?f21");
        xmlWriter->addAttribute("draw:type", "non-primitive");
        ...
        xmlWriter->startElement("draw:enhanced-geometry");
        xmlWriter->addAttribute("draw:enhanced-path",
                                "M 1152145 0 C 1563767 0 1944120 219599 2149931 576074 2355741 932549 2355740 1371744 2149929 1728219 L 1152144 1152144 C "
                                "1152144 768096 1152145 384048 1152145 0 Z N");
        xmlWriter->addAttribute("draw:glue-point-leaving-directions", "-90, -90, -90, -90, -90");
        ...
        xmlWriter->addAttribute("draw:formula", "left");
        xmlWriter->addAttribute("draw:name", "f0");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "right");
        xmlWriter->addAttribute("draw:name", "f1");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "top");
        xmlWriter->addAttribute("draw:name", "f2");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "bottom");
        xmlWriter->addAttribute("draw:name", "f3");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "?f3 - ?f2");
        xmlWriter->addAttribute("draw:name", "f4");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "?f4 / 2");
        xmlWriter->addAttribute("draw:name", "f5");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "?f4 / 4");
        xmlWriter->addAttribute("draw:name", "f6");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "?f2 + ?f5");
        xmlWriter->addAttribute("draw:name", "f7");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "?f1 - ?f0");
        xmlWriter->addAttribute("draw:name", "f8");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "?f8 / 2");
        xmlWriter->addAttribute("draw:name", "f9");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "?f8 / 4");
        xmlWriter->addAttribute("draw:name", "f10");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "?f0 + ?f9");
        xmlWriter->addAttribute("draw:name", "f11");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "?f8 * 3 / 4");
        xmlWriter->addAttribute("draw:name", "f12");
        xmlWriter->endElement();
        xmlWriter->startElement("draw:equation");
        xmlWriter->addAttribute("draw:formula", "?f4 * 3 / 4");
        ...
        if (endstyle == NoArrow) { // just a connecting line without arrow
            xmlWriter->startElement("draw:enhanced-geometry");
            xmlWriter->addAttribute("draw:enhanced-path",
                                    "M 1627875 92938 A ?f54 ?f55 ?f56 ?f57 1627875 92938 ?f51 ?f53  W ?f58 ?f59 ?f60 ?f61 1627875 92938 ?f51 ?f53 N");
            // xmlWriter->addAttribute("draw:glue-point-leaving-directions", "-90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90");
            // xmlWriter->addAttribute("draw:glue-points", "?f20 ?f21 ?f22 ?f23 ?f24 ?f25 ?f26 ?f25 ?f27 ?f23 ?f28 ?f21 ?f28 ?f29 ?f27 ?f30 ?f26 ?f31 ?f24 ?f31
            // ?f22 ?f30 ?f20 ?f29 ?f20 ?f21");
            xmlWriter->addAttribute("draw:type", "non-primitive");
...
QString PresentationOfAtom::dump() const
{
    QString s;
    if (!m_axis.isEmpty())
        s += QString("axis=%1 ").arg(m_axis);
    if (!m_ptType.isEmpty())
        s += QString("ptType=%1 ").arg(m_ptType);
    if (!m_start.isEmpty())
        s += QString("start=%1 ").arg(m_start);
    if (!m_step.isEmpty())
        s += QString("step=%1 ").arg(m_step);
    if (!m_count.isEmpty())
        s += QString("count=%1 ").arg(m_count);
    if (!m_hideLastTrans.isEmpty())
        s += QString("hideLastTrans=%1 ").arg(m_hideLastTrans);
    return s.trimmed();
}
...
void IfAtom::dump(Context *context, int level)
{
    ...
    // DEBUG_DUMP << "name=" << m_name << "argument=" << m_argument << "axis=" << m_axis << "count=" << m_count << "function=" << m_function << "hideLastTrans="
    // << m_hideLastTrans << "operator=" << m_operator << "dataPointType=" << m_ptType << "start=" << m_start << "step=" << m_step << "value=" << m_value;
    AbstractAtom::dump(context, level);
}
...
QString ForEachAtom::dump() const
{
    QString s;
    if (!m_name.isEmpty())
        s += QString("name=%1 ").arg(m_name);
    if (!m_axis.isEmpty())
        s += QString("axis=%1 ").arg(m_axis);
    if (!m_ptType.isEmpty())
        s += QString("ptType=%1 ").arg(m_ptType);
    if (!m_reference.isEmpty())
        s += QString("reference=%1 ").arg(m_reference);
    if (!m_start.isEmpty())
        s += QString("start=%1 ").arg(m_start);
    if (!m_step.isEmpty())
        s += QString("step=%1 ").arg(m_step);
...
Context *AbstractAlgorithm::context() const
{
    return m_context;
}
LayoutNodeAtom *AbstractAlgorithm::layout() const
{
    return m_layout.data();
}
LayoutNodeAtom *AbstractAlgorithm::parentLayout() const
{
    return m_parentLayout.data();
}
...
void AbstractAlgorithm::virtualDoLayout()
{
    ...
    //     setNodePosition( curChild.data(), layout()->finalValues()[ "l" ], layout()->finalValues()[ "t" ], layout()->finalValues()[ "w" ],
    //     layout()->finalValues()[ "h" ] );
...
qreal CompositeAlgorithm::virtualGetDefaultValue(const QString &type, const QMap<QString, qreal> &values)
{
    ...
    if (type == "w" || type == "h") {
        ...
        value = 0;
    } else if (type == "wOff" || type == "hOff" || type == "lOff" || type == "ctrXOff" || type == "rOff" || type == "tOff" || type == "ctrYOff"
               || type == "bOff") {
        value = 0;
    }
    return value;
...
void CycleAlgorithm::virtualDoLayout()
{
    ...
    // Specifies the angle at which the first shape is placed. Angles are in degrees, measured clockwise from a line pointing straight upward from the center of
    // the cycle.
    int startAngel = layout()->algorithmParam("stAng", "0").toInt();
    // Specifies the angle the cycle spans. Final shapealign text is placed at stAng+spanAng, unless spanAng=360. In that case, the algorithm places the text so
...
void SnakeAlgorithm::virtualDoLayout()
{
    ...
    foreach (LayoutNodeAtom *l, childs) {
        if (l->algorithmType() == AlgorithmAtom::SpaceAlg)
            continue; // specs says 'or does nothing' but not under which conditions :-/
        setNodePosition(l, x, y, dw, dh);
        if (!inSameDirection)
            inRows = !inRows;
        if (inRows) {

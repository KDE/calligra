    ...
    if (reader->isStartElement()) {
        ...
        if (reader->qualifiedName() == QLatin1String("dgm:layoutNode")) {
            ...
            while (!reader->atEnd()) {
                ...
                if (!reader->isStartElement() && reader->qualifiedName() == "dgm:varLst") {
                    break;
                }
                if (reader->isStartElement()) {
                    const QXmlStreamAttributes attrs(reader->attributes());
...
void PresentationOfAtom::build(Context *context)
{
    ...
    if (nodes.isEmpty()) {
        ...
        */
        ...
        if (nodes.count() >= 2) {
            kWarning() << "TODO The axis contains more then one note. It's not clear what to do in such cases...";
        }
...
{
    return m_isTrue;    // is true or false?
}

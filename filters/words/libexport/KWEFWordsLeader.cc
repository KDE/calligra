                               ...
                               QList<ParaData>& paraList)
{
    ...
    QList<ParaData>::Iterator paraIt;
    ...
    for (paraIt = paraList.begin(); paraIt != paraList.end(); ++paraIt) {
        ...
        for (formattingIt = (*paraIt).formattingList.begin();
            ...
            if ((*formattingIt).id              == 6    &&
                ...
                kDebug(30508) << "Found anchor" << (*formattingIt).frameAnchor.key.toString();
...
static void ProcessHardBrkTag(QDomNode myNode, void* tagData, KWEFWordsLeader*)
{
    // <HARDBRK>
    ...
    if (*flag)
        kDebug(30508) << "<HARDBRK frame=\"1\"> found";
}
...
void KWEFWordsLeader::createBookmarkFormatData(ParaData& paraData)
{
...
{
#if 0
    kDebug(30508) << "ProcessParagraphTag () - Begin";
#endif
...
#if 0
    kDebug(30508) << "ProcessParagraphTag () - End" << paraText;
#endif
}
...
static void ProcessPictureAnchor(QDomNode myNode, KWEFWordsLeader *leader, FrameAnchor* frameAnchor, const int frameType)
{
    ...
    kDebug(30508) << "FRAMESET PICTURE KEY" << frameAnchor->picture.key.toString();

    frameAnchor->key = frameAnchor->picture.key;
}

static void ProcessTableAnchor(QDomNode myNode, KWEFWordsLeader *leader, FrameAnchor* frameAnchor,
                               const int col, const int row, const int cols, const int rows)
{
    frameAnchor->type = 6; // Table

    QList<ParaData> cellParaList;
    QList<TagProcessing> tagProcessingList;
...
{
#if 0
    kDebug(30508) << "ProcessFramesetTag () - Begin";
#endif
    ...
    case 1:
        if (grpMgr.isEmpty()) {
            ...
            if (frameInfo == 0) {
                // Normal Text
                kDebug(30508) << "Processing Frameset:" << leader->m_currentFramesetName;
                QList<TagProcessing> tagProcessingList;
                ...
                if (cols > 0 && rows > 0) {
                    ...
                    if (frameAnchor) {
                        ...
                        for (it = leader->m_nonInlinedTableAnchors.begin(); it !=  leader->m_nonInlinedTableAnchors.end(); ++it) {
                            if ((*it).key == key) {
                                kDebug(30508) << "Found pseudo-anchor for table:" << (*it).key.toString();
                                found = true;
                                break;
                            }
                        }
    ...
    case 5: { // CLIPART: deprecated (up to Calligra 1.2 Beta 2)
        ...
        if (frameAnchor) {
            ...
            kDebug(30508) << "ProcessFramesetTag: Couldn't find anchor" << leader->m_currentFramesetName;
            FrameAnchor anchor;
...
#if 0
    kDebug(30508) << "ProcessFramesetTag () - End";
#endif
}
...
static void ProcessStyleTag(QDomNode myNode, void *, KWEFWordsLeader *leader)
{
    LayoutData layout;
...
static void ProcessStylesPluralTag(QDomNode myNode, void *, KWEFWordsLeader *leader)
{
    AllowNoAttributes(myNode);
...
static void ProcessPaperBordersTag(QDomNode myNode, void*, KWEFWordsLeader* leader)
{
...
static void ProcessPaperTag(QDomNode myNode, void *, KWEFWordsLeader *leader)
{
...
static void ProcessVariableSettingsTag(QDomNode myNode, void *, KWEFWordsLeader *leader)
{
    ...
    // ( e.g. syntax 2 file calligratests/documents/export/words/text/all.kwd )
    attrProcessingList
    ...
    //kDebug(30508) <<"Creation time:" << vs.creationTime.toString( Qt::ISODate );
    ...
    //kDebug(30508) <<"Modification time:" << vs.modificationTime.toString( Qt::ISODate );

    if (!print.isEmpty())
        vs.printTime = QDateTime::fromString(print, Qt::ISODate);
    //kDebug(30508) <<"Print time:" << vs.printTime.toString( Qt::ISODate );

    leader->doVariableSettings(vs);
}

static void ProcessSpellCheckIgnoreWordTag(QDomNode myNode, void *, KWEFWordsLeader *leader)
{
    QString ignoreword;
...
static void ProcessSpellCheckIgnoreListTag(QDomNode myNode, void *, KWEFWordsLeader *leader)
{
    AllowNoAttributes(myNode);
                                 ...
                                 KWEFWordsLeader *leader)
{
    ...
    kDebug(30508) << "search anchors:" << key.toString();
    bool found = false;
    ...
    for (paraIt = paraList->begin(); paraIt != paraList->end(); ++paraIt) {
        ...
        for (formattingIt = (*paraIt).formattingList.begin();
            ...
            if ((((*formattingIt).id == 6) || ((*formattingIt).id == 2))
                    && (*formattingIt).frameAnchor.key == key) {
                kDebug(30508) << "Found anchor for inlined picture:" << (*formattingIt).frameAnchor.key.toString();
    ...
    for (it = leader->m_nonInlinedPictureAnchors.begin(); it !=  leader->m_nonInlinedPictureAnchors.end(); ++it) {
        if ((*it).key == key) {
            kDebug(30508) << "Found pseudo-anchor for non-inlined picture:" << (*it).key.toString();
...
static void FreeCellParaLists(QList<ParaData> &paraList)
{
    QList<ParaData>::Iterator paraIt;
    ...
    for (paraIt = paraList.begin(); paraIt != paraList.end(); ++paraIt) {
        ...
        for (formattingIt = (*paraIt).formattingList.begin();
            ...
            if ((*formattingIt).id == 6 && (*formattingIt).frameAnchor.type == 6) {
                QList<TableCell>::Iterator cellIt;
...
static void ProcessFootnoteFramesetTag(QDomNode myNode, void *tagData, KWEFWordsLeader *leader)
{
    Q_UNUSED(tagData);
    QString frameName;
...
static void ProcessFootnoteFramesetsTag(QDomNode myNode, void *tagData, KWEFWordsLeader *leader)
{
    AllowNoAttributes(myNode);
...
static void ProcessBookmarkItemTag(QDomNode myNode, void* tag, KWEFWordsLeader *leader)
{
    QList<Bookmark> * bookmarkList = static_cast< QList<Bookmark> * >(tag);
    ...
    kDebug(30508) << "Bookmark:" << bookmark.m_name << " in frameset" << bookmark.m_frameset;

    bookmarkList->append(bookmark);
}

static void ProcessBookmarksTag(QDomNode myNode, void* tag, KWEFWordsLeader *leader)
{
    AllowNoAttributes(myNode);
...
void ProcessDocTag(QDomNode         myNode,
                   void* /*tagData*/, KWEFWordsLeader* leader)
{
    //kDebug (30508) <<"Entering ProcessDocTag";
    ...
    kDebug(30508) << "Document written by" << editor;
    kDebug(30508) << "Document of syntax version" << leader->m_syntaxVersion;
    ...
    if (leader->m_syntaxVersion == 1) {
        ...
        if (author == "Reginald Stadlbauer and Torben Weis") {
            kDebug(30508) << "No syntax version but author attribute matches => assuming old syntax";
    ...
    if (nodeIgnoreList.isNull())
        kDebug(30508) << "No <SPELLCHECKIGNORELIST>"; // Most files will not have it!
    else
    ...
    // TODO: why are the followings used by Words 1.2 but are not in its DTD?
    ...
    kDebug(30508) << "Unachored Framesets : END";

    FreeCellParaLists(paraList);

    leader->doCloseBody();

    //kDebug (30508) <<"Exiting ProcessDocTag";
}


void KWEFWordsLeader::setWorker(KWEFBaseWorker *newWorker)
{
    m_worker = newWorker;

    if (newWorker)
        newWorker->registerWordsLeader(this);
}


KWEFBaseWorker *KWEFWordsLeader::getWorker(void) const
{
    return m_worker;
}
...
bool KWEFWordsLeader::doOpenFile(const QString &filenameOut, const QString &to)
{
    if (m_worker)
        return m_worker->doOpenFile(filenameOut, to);

    // As it would be the first method to be called, warn if worker is NULL
    kError(30508) << "No Worker! (in KWEFWordsLeader::doOpenFile)" << endl;
...
bool KWEFWordsLeader::doFullDocumentInfo(const KWEFDocumentInfo &docInfo)
{
    if (m_worker)
...
bool KWEFWordsLeader::doVariableSettings(const VariableSettingsData &varSettings)
{
    if (m_worker)
...
bool KWEFWordsLeader::doFullDocument(const QList<ParaData> &paraList)
{
    if (m_worker)
...
bool KWEFWordsLeader::doPageInfo(const int headerType, const int footerType)
{
    if (m_worker)
...
bool KWEFWordsLeader::doFullPaperFormat(const int format, const double width, const double height, const int orientation)
{
    if (m_worker)
...
bool KWEFWordsLeader::doFullPaperBorders(const double top, const double left, const double bottom, const double right)
{
    if (m_worker)
...
bool KWEFWordsLeader::doFullPaperFormatOther(const int columns, const double columnspacing, const int numPages)
{
    if (m_worker)
...
bool KWEFWordsLeader::doFullDefineStyle(LayoutData &layout)
{
    if (m_worker)
...
bool KWEFWordsLeader::doFullSpellCheckIgnoreWord(const QString& ignoreword)
{
    if (m_worker)
...
bool KWEFWordsLeader::doHeader(const HeaderData& header)
{
    if (m_worker)
...
bool KWEFWordsLeader::doFooter(const FooterData& footer)
{
    if (m_worker)
...
bool KWEFWordsLeader::doDeclareNonInlinedFramesets(QList<FrameAnchor>& pictureAnchors, QList<FrameAnchor>& tableAnchors)
{
    if (m_worker)
...
static bool ParseFile(QIODevice* subFile, QDomDocument& doc)
{
    ...
    if (!doc.setContent(subFile, &errorMsg, &errorLine, &errorColumn)) {
        ...
        KMessageBox::error(0L, i18n("An error has occurred while parsing the Words file.\nAt line: %1, column %2\nError message: %3", errorLine , errorColumn , i18n("QXml", errorMsg)),
                           i18n("Words Export Filter Library"), 0);
        return false;
    }
    return true;
}
                             ...
                             KWEFWordsLeader* leader)
{
    if (!subFile) {
        ...
        kDebug(30508) << "Processing Document...";
        QDomDocument doc;
...
QIODevice* KWEFWordsLeader::getSubFileDevice(const QString& fileName)
{
...
bool KWEFWordsLeader::loadSubFile(const QString& fileName, QByteArray& array)
{
    KoStoreDevice* subFile;
...
KoFilter::ConversionStatus KWEFWordsLeader::convert(KoFilterChain* chain,

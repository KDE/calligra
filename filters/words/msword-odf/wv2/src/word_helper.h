...
public:
    ...
    unsigned int extraDataLength() const
    {
        return m_extraDataLength;
    }
...
template<class T, bool shortCount = false>
class PLF
{
public:
...
PLCF<NewT> *convertPLCF(const PLCF<OldT> &old); // evil, eh? :-)
...
class PLCF
{
    ...
    template<typename OldT, typename NewT>
    friend PLCF<NewT> *convertPLCF(const PLCF<OldT> &old);
...
template<typename OldT, typename NewT>
PLCF<NewT> *convertPLCF(const PLCF<OldT> &old)
{
...
FKP<Offset>::FKP(const FKP<Offset> &rhs)
    : m_crun(rhs.m_crun)
    , m_internalOffset(rhs.m_internalOffset)
{
    m_rgfc = new U32[m_crun + 1];
...
public:
    ...
    void toLast()
    {
        m_index = m_fkp.m_crun - 1;
    }

    U32 currentStart() const
    {
        if (m_index < m_fkp.m_crun)
            return m_fkp.m_rgfc[m_index];
        return 0;
    }
    U32 currentLim() const
    {
        if (m_index < m_fkp.m_crun)
            return m_fkp.m_rgfc[m_index + 1];
        return 0;
    }
    Offset currentOffset() const
    {
        if (m_index < m_fkp.m_crun)
            return m_fkp.m_rgb[m_index];
        return Offset();
    }
...
template<typename PHE>
const unsigned int BX<PHE>::sizeOf = 1 + PHE::sizeOf;

template<typename PHE>
bool operator==(const BX<PHE> &lhs, const BX<PHE> &rhs)
{

...
U16 determineParameterLength(U16 sprm, const U8 *in, WordVersion version)
{
    if (version == Word8) {
        ...
        if (sprm > 255)
            wvlog << "Error: Trying to get the length of a flaky SPRM (" << sprm << ", 0x" << Qt::hex << sprm << Qt::dec << ") via the Word 95 method!"
                  << Qt::endl;
...
void PAP::apply(const U8 *grpprl, U16 count, const Style *style, const StyleSheet *styleSheet, OLEStreamReader *dataStream, WordVersion version)
{
...
struct Zone {
    ...
    bool contains(S16 position) const
    {
        return m_center - m_plusMinus <= position && m_center + m_plusMinus >= position;
    }
...
S16 PAP::applyPAPSPRM(const U8 *ptr, const Style *style, const StyleSheet *styleSheet, OLEStreamReader *dataStream, WordVersion version)
{
    ...
    case SPRM::sprmPChgTabsPapx: {
        ...
        if (cch != 1 + 2 * itbdDelMax + 1 + 3 * itbdAddMax) {
            wvlog << "Offset problem in sprmPChgTabsPapx. cch=" << static_cast<int>(cch) << "data size=" << 1 + 2 * itbdDelMax + 1 + 3 * itbdAddMax << Qt::endl;
        }
#ifdef WV2_DEBUG_SPRMS
    ...
    case SPRM::sprmPChgTabs: {
        ...
        if (cch != 255 && cch != 1 + 4 * itbdDelMax + 1 + 3 * itbdAddMax) {
            wvlog << "Offset problem in sprmPChgTabs. cch=" << static_cast<int>(cch) << "data size=" << 1 + 4 * itbdDelMax + 1 + 3 * itbdAddMax << Qt::endl;
        }
#ifdef WV2_DEBUG_SPRMS
...
void CHP::apply(const U8 *grpprl, U16 count, const Style *paragraphStyle, const StyleSheet *styleSheet, OLEStreamReader *dataStream, WordVersion version)
{
...
#ifdef WV2_DEBUG_SPRMS
        wvlog << "sprmCFBold operand: 0x" << hex << *ptr << "| istd: 0x" << hex << istd << "| paragraphStyle:" << paragraphStyle;
#endif
        if (*ptr < 128) {
            fBold = *ptr == 1;
...
void PICF::apply(const U8 *grpprl, U16 count, const Style *style, const StyleSheet *styleSheet, OLEStreamReader *dataStream, WordVersion version)
{
...
// of the applied sprm if it was successful
S16 PICF::applyPICFSPRM(const U8 *ptr, const Style * /*style*/, const StyleSheet * /*styleSheet*/, OLEStreamReader * /*dataStream*/, WordVersion version)
{
    U16 sprmLength;
    const U16 sprm(getSPRM(&ptr, version, sprmLength));
    ...
    case SPRM::sprmPicScale:
        if (*ptr != 12)
            wvlog << "Warning: sprmPicScale has a different size (" << static_cast<int>(*ptr) << ") than expected (12)." << Qt::endl;
        mx = readU16(ptr + 1);
        my = readU16(ptr + 3);
...
void SEP::apply(const U8 *grpprl, U16 count, const Style *style, const StyleSheet *styleSheet, OLEStreamReader *dataStream, WordVersion version)
{
...
void TAP::apply(const U8 *grpprl, U16 count, const Style *style, const StyleSheet *styleSheet, OLEStreamReader *dataStream, WordVersion version)
{
...
S16 TAP::applyTAPSPRM(const U8 *ptr, const Style *style, const StyleSheet *styleSheet, OLEStreamReader *dataStream, WordVersion version)
{
    ...
    if (((sprm & 0x1C00) >> 10) != 5 && (sprm != SPRM::sprmPHugePapx) && (sprm != SPRM::sprmPHugePapx2) && (sprm != SPRM::sprmPTableProps)) {
#ifdef WV2_DEBUG_SPRMS
        wvlog << "Warning: You're trying to apply a non TAP sprm to a TAP. Not necessarily bad." << Qt::endl;
#endif
        return -1;
    }
    // which one? ;)
    ...
    case SPRM::sprmTInsert: {
        ...
        if (itcMac < itcFirst) {
            ...
            std::for_each(rgdxaCenter.begin() + itcFirst + ctc, rgdxaCenter.end(), [=](S16 &item) {
                item += ctc * dxaCol;
            });
        }
        break;
    ...
    case SPRM::sprmTDxaCol: {
        ...
        ++itcFirst;
        std::for_each(rgdxaCenter.begin() + itcFirst, rgdxaCenter.end(), [=](S16 &item) {
            item -= shift;
        });
        break;
    }
    ...
    case SPRM::sprmTCellPaddingDefault:
        if (*ptr == 6) {
            ...
            wvlog << "Warning: sprmTCellPaddingDefault with unusual length=" << static_cast<int>(*ptr) << Qt::endl;
        }
        break;

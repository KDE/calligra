...
private:
    ...
    void processPiece(String *string, U32 fc, U32 limit, const Position &position);
    ...
    void processChunk(const Chunk &chunk, SharedPtr<const Word97::CHP> chp, U32 length, U32 index, U32 currentStart);
    void processRun(const Chunk &chunk, SharedPtr<const Word97::CHP> chp, U32 length, U32 index, U32 currentStart);
     ...
     * the ODF container.
     */
    QString emitPictureData(const U32 globalCP, SharedPtr<const Word97::CHP> chp, const bool isBulletPicture = false);
    ...
    struct ParsingState {
        ParsingState(Position *tableRowS,
                     U32 tableRowL,
                     bool cMarkFound,
                     int remCells,
                     bool ts,
                     ...
                     SubDocument subD,
                     ParsingMode mode)
            : tableRowStart(tableRowS)
            , tableRowLength(tableRowL)
            , cellMarkFound(cMarkFound)
            , remainingCells(remCells)
            , tableSkimming(ts)
            , paragraph(parag)
            , remainingChars(remChars)

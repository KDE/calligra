...
public:
    inline QDomDocument getDocInfo(void) const {
        return m_info;
    }
    inline QDomDocument getDocument(void) const {
        return mainDocument;
    }
    inline bool wasFatalError(void) const {
        return m_fatalerror;
    }
protected:
...
bool StructureParser::StartElementC(StackItem* stackItem, StackItem* stackCurrent, const QXmlAttributes& attributes)
{
    ...
    } else {//we are not nested correctly, so consider it a parse error!
...
{
    if (stackItem->elementType == ElementTypeContent) {  // Normal <c>
                                        ...
                                        const QXmlAttributes& attributes)
{
    ...
    if ((stackCurrent->elementType != ElementTypeParagraph) && (stackCurrent->elementType != ElementTypeContent)) {//we are not nested correctly, so consider it a parse error!
...
bool StructureParser :: characters(const QString & ch)
{
    ...
    if ((stackItem->elementType == ElementTypeContent)
            || (stackItem->elementType == ElementTypeAnchorContent)) { // <c>
        success = charactersElementC(stackItem, mainDocument, ch);
    } else if (stackItem->elementType == ElementTypeParagraph) { // <p>
        success = charactersElementP(stackItem, mainDocument, ch);
    } else if (stackItem->elementType == ElementTypeAnchor) { // <a>

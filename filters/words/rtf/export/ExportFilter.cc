...
// map Words field name to RTF field name
// e.g authorName -> AUTHOR
static QString mapFieldName(const QString& wordsField)
{
    QString rtfField;

    if (wordsField == "fileName") rtfField = "FILENAME";
    else if (wordsField == "authorName") rtfField = "AUTHOR";
    else if (wordsField == "docTitle") rtfField = "TITLE";
            ...
            if (1 == (*paraFormatDataIt).id) {
                ...
                if ((0 == (*paraFormatDataIt).variable.m_type) // date
                    ...
                    if (key.startsWith("locale")) {
                        ...
                        // KLocale's key differ from Words
...
bool RTFWorker::doFullDocumentInfo(const KWEFDocumentInfo& docInfo)
{
    ...
    m_textDocInfo += "Generated by Words's RTF Export Filter";
...
// The following function encodes the words unicode characters into
                               ...
                               const LayoutData& layout, const bool force)
{
    ...
    // - Words's RTF import filter makes the page break immediately (also in styles)
    ...
    if (force
        ...
        if (layout.lineSpacingType == LayoutData::LS_SINGLE)
            ...
            // "Custom" in Words is like "Exactly" in MS Word
            strLayout += QString("\\sl-%1\\slmult0").arg(int(layout.lineSpacing) * 20);
    ...
    // shadow support
    // note shadow in Words is more full-feature/sophisticated than RTF
    // here we just treat Words's shadow as simple \shad mark-up

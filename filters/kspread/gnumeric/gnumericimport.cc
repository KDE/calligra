...
void GNUMERICFilter::GnumericDate::jul2greg( double num, int & y, int & m, int & d )
{
  ...
  kdDebug() << "***** Num: " << num << ", i: " << i  << endl;

  QDate::julianToGregorian( i + g_dateOrigin, y, m, d );
  kdDebug() << "y: " << y << ", m: " << m << ", d: " << d << endl;
}
...
QTime GNUMERICFilter::GnumericDate::getTime( double num )
{
  ...
  kdDebug() << "***** Num: " << num << ", secs " << secs << endl;
  ...
  kdDebug() << "****** h: " << h << ", m: " << m << ", secs: " << secs << endl;
...
void setColInfo(QDomNode * sheet, KSpreadSheet * table)
{
  ...
  if ( def.hasAttribute( "DefaultSizePts" ) )
  {
    ...
    if ( ok )
    {
      KSpreadLayout::setGlobalColWidth( d );
...
void setRowInfo(QDomNode *sheet, KSpreadSheet *table)
{
  ...
  if ( def.hasAttribute( "DefaultSizePts" ) )
  {
    ...
    if ( ok )
    {
      KSpreadLayout::setGlobalRowHeight( d );
                              ...
                              QString & cell_content )
{
  ...
  {
    kdDebug() << "Cell_format: " << cell_format_date[i] << ", FormatString: " << formatString << endl;
    if ( ( formatString == "d/m/yy" ) || ( formatString == cell_format_date[i] ) )
    {
      kdDebug() << "   FormatString: Date: " << formatString << ", CellContent: " << cell_content << endl;
      QDate date;
      if ( !kspread_cell->isDate() )
      {
        ...
        kdDebug() << "!!!   FormatString: Date: " << formatString << ", CellContent: " << cell_content 
                  << ", Double: " << val << endl;
        if ( !ok )
          return false;
        
        GnumericDate::jul2greg( val, y, m, d );
        kdDebug() << "     num: " << val << ", y: " << y << ", m: " << m << ", d: " << d << endl;
  ...
  for ( i = 0; cell_format_time[i] ; ++i )
  {
    if (formatString == cell_format_time[i])
    {
      ...
      if ( !kspread_cell->isTime() )
      {
        ...
        kdDebug() << "   FormatString: Time: " << formatString << ", CellContent: " << cell_content 
                  << ", Double: " << content << endl;
...
QString GNUMERICFilter::convertVars( QString const & str, KSpreadSheet * table ) const
{
  ...
  for ( uint i = 0; i < count; ++i )
  {
    ...
    if ( n != -1 )
    {
      kdDebug() << "Found var: " << list1[i] << endl;
      if ( i == 0 )
...
void GNUMERICFilter::ParsePrintInfo( QDomNode const & printInfo, KSpreadSheet * table )
{
  ...
  if ( !foot.isNull() )
  {
    kdDebug() << "Parsing footer: " << foot.attribute("Left") << ", " << foot.attribute("Middle") << ", "
  ...
  if ( !head.isNull() )
  {
    kdDebug() << "Parsing header: " << head.attribute("Left") << ", " << head.attribute("Middle") << ", "
  ...
  table->setPaperLayout( fleft, ftop, fright, fbottom,
...
void GNUMERICFilter::ParseFormat(QString const & formatString, KSpreadCell * kspread_cell)
{
  ...
  if (formatString[l - 1] == '%')
  {
    kspread_cell->setFormatType(KSpreadLayout::Percentage);
    kspread_cell->setFactor(100);
  }
  else if (formatString[0] == '$')
  {
    kspread_cell->setFormatType(KSpreadLayout::Money); 
  ...
  else if (l > 1)
  {
    ...
    {
      kspread_cell->setFormatType(KSpreadLayout::Scientific);
    }
    else
    {
      ...
      {
        // TODO: fixme!
        kspread_cell->setFormatType( KSpreadLayout::fraction_three_digits );
        return;
      }
  ...
  if (formatString.find('(', lastPos) != -1)
  {
    if ( red )
      ...
      kspread_cell->setFloatColor( KSpreadLayout::NegBrackets );
  }
}
      ...
      for ( column = startCol; column <= endCol; ++column )
      {
        ...
        {          
          ...
          if (style_element.hasAttribute("Format"))
          {
            ...
            kdDebug() << "Format: " << formatString << endl;
	...
	while ( !cell.isNull() ) 
	  ...
	  if ( !e.isNull() ) 
		...
		if (e.hasAttribute("ExprID"))
		    ...
		    qDebug("%s", 
                           table->cellAt( column, row, false )->decodeFormula( expr, column, row ).latin1());
		    qDebug("%s", expr);
	  ...
	  cell = cell.nextSibling();
        ...
        kdDebug() << "Reading in cells done" << endl;

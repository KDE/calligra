...
public:
    ...
    virtual void setData(unsigned size, const unsigned char* data, const unsigned int* /*continuePositions*/) {
        ...
        if(m_worksheetHandler) {
            ...
            m_formula = m_worksheetHandler->decodeFormula(0, 0, true, tokens);
        } else {
            FormulaTokens tokens = m_handler->globals()->decodeFormula(size, 6, data, version());
            m_formula = m_handler->globals()->decodeFormula(0, 0, true, tokens);
        }
    }
private:
...
ChartSubStreamHandler::ChartSubStreamHandler(GlobalsSubStreamHandler* globals, SubStreamHandler* parentHandler)
    : SubStreamHandler(), m_globals(globals), m_parentHandler(parentHandler), m_chart(new Chart()), m_sheet(0)
{
...
void ChartSubStreamHandler::handleBegin(BeginRecord *) { // secifies the begin of a collection of records
    //std::cout << "ChartSubStreamHandler::handleBegin" << std::endl;
}
void ChartSubStreamHandler::handleEnd(EndRecord *) { // sepcified the end of a collection of records
...
void ChartSubStreamHandler::handleSeries(SeriesRecord *record) { // series, trendline or errorchars
    if(!record) return;
...
void ChartSubStreamHandler::handleChart3d(Chart3dRecord *record) { // specifies that chart is rendered in 3d scene
    if(!record) return;
...
void ChartSubStreamHandler::handleSerToCrt(SerToCrtRecord *record) { // specifies the chartgroup for the current series
    if(!record) return;
    std::cout << "ChartSubStreamHandler::handleSerToCrt id=" << record->identifier() << std::endl;
}
void ChartSubStreamHandler::handleShtProps(ShtPropsRecord *) { // properties
    //TODO
}
void ChartSubStreamHandler::handleDefaultText(DefaultTextRecord *record) { // text
    if(!record) return;
    std::cout << "ChartSubStreamHandler::handleDefaultText id=" << record->identifier() << std::endl;
    //TODO
}
void ChartSubStreamHandler::handleText(TextRecord *record) { // text formatting
    if(!record) return;
...
void ChartSubStreamHandler::handlePie(PieRecord *record) { // specifies that the chartgroup is a pie chart
    if(!record) return;
...
void ChartSubStreamHandler::handleSIIndex(SIIndexRecord *record) { // type of data contained in the Number records following
    if(!record) return;

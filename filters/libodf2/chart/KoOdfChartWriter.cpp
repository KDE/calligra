...
QString KoOdfChartWriter::normalizeCellRange(QString range)
{
    ...
    QRegularExpression regEx(isPoint ? "(|.*\\.|.*\\!)([A-Z0-9]+)" : "(|.*\\.|.*\\!)([A-Z]+[0-9]+)\\:(|.*\\.|.*\\!)([A-Z0-9]+)");
...
QString KoOdfChartWriter::generateGradientStyle(KoGenStyles &mainStyles, const KoChart::Gradient *grad)
{
...
QString KoOdfChartWriter::genChartAreaStyle(KoGenStyle &style, KoGenStyles &styles, KoGenStyles &mainStyles)
{
...
QString KoOdfChartWriter::genPlotAreaStyle(KoGenStyle &style, KoGenStyles &styles, KoGenStyles &mainStyles)
{
...
void KoOdfChartWriter::addShapePropertyStyle(/*const*/ KoChart::Series *series, KoGenStyle &style, KoGenStyles & /*mainStyles*/)
{
    Q_ASSERT(series);
    bool marker = false;
    ...
    } else if ((paletteIsSet && m_chart->m_impl->name() != "scatter") || m_chart->m_showLines) {
    ...
    } else if (paletteIsSet && !(m_chart->m_markerType != KoChart::NoMarker || marker) && series->m_markerType == KoChart::NoMarker) {
...
QString KoOdfChartWriter::replaceSheet(const QString &originalString, const QString &replacementSheet)
{
...
bool KoOdfChartWriter::saveContent(KoStore *store, KoXmlWriter *manifestWriter)
{
    ...
    if (chart()->m_is3d) {
        // bodyWriter->addAttribute("dr3d:transform", "matrix (0.893670830886674 0.102940425033731 -0.436755898547686 -0.437131441492021 0.419523087196176
        // -0.795560483036015 0.101333848646097 0.901888933407692 0.419914042293545 0cm 0cm 0cm)"); bodyWriter->addAttribute("dr3d:vrp", "(12684.722548717
        // 7388.35827488833 17691.2795565958)"); bodyWriter->addAttribute("dr3d:vpn", "(0.416199821709347 0.173649045905254 0.892537795986984)");
    ...
    //         if (sheetReplacement)
    //             bodyWriter->addAttribute("table:cell-range-address", replaceSheet(normalizeCellRange(m_cellRangeAddress), QString::fromLatin1("local")));
    //             //"Sheet1.C2:Sheet1.E5");
    //         else
...
bool KoOdfChartWriter::saveSeries(KoGenStyles &styles, KoGenStyles &mainStyles, KoXmlWriter *bodyWriter, int maxExplode)
{
    int curSerNum = 0;
    bool lines = true;
    bool marker = false;
    Q_FOREACH (KoChart::Series *series, chart()->m_series) {
        ...
        if (paletteIsSet && m_chart->m_impl->name() != "ring" && m_chart->m_impl->name() != "circle") {
        ...
        if (series->m_datasetValue.contains(KoChart::Value::SeriesLegendOrTrendlineName)) {
            ...
            if (!v->m_formula.isEmpty()) {
                bodyWriter->addAttribute("chart:label-cell-address",
                                         (v->m_type == KoChart::Value::CellRange ? normalizeCellRange(v->m_formula) : v->m_formula));
            }
        }
...
void KoOdfChartWriter::addDataThemeToStyle(KoGenStyle &style, int dataNumber, int maxNumData, bool strokes)
{

...
public:
    ...
    bool m_isUnlinkedFormat; ///< false=data uses the number formatting of the referenced data, true=data uses the custom formatting specified via
                             ///< m_numberFormat.
    ...
    Value(DataId dataId, DataType type, const QString &formula = QString(), bool isUnlinkedFormat = false, unsigned numberFormat = 0)
        : m_dataId(dataId)
        , m_type(type)
...
public:
    ...
    explicit AreaFormat(const QColor &foreground = QColor(), const QColor &background = QColor(), bool fill = false)
        : Format()
...
public:
    ...
    QByteArray name() const override
    {
        return m_filled ? "filled-radar" : "radar";
    }
};
...
public:
    ...
    explicit BubbleImpl(SizeType sizeType = Area, unsigned int sizeRatio = 100, bool showNegativeBubbles = true)
        : ChartImpl()
        , m_sizeType(sizeType)
...
public:
    ...
    explicit SurfaceImpl(bool fill = false)
        : ChartImpl()
        , m_fill(fill)
    {
    }

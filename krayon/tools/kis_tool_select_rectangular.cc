...
void RectangularSelectTool::dragSelectImage( QPoint dragPoint )
{
    KisImage *img = m_pDoc->current();
    if ( !img )
        return;
...
bool RectangularSelectTool::pasteClipImage( QPoint pos )
{
    KisImage *img = m_pDoc->current();
    if ( !img )
        return false;

...
bool Calendar::load(KoXmlElement &element, XMLLoaderObject &status)
{
    ...
    if (tz.isValid()) {
        ...
        kWarning() << "No timezone specified, use default (local)";
    }
...
//     kDebug(planDbg())<<m_name<<":"<<start<<start.timeSpec()<<"to"<<end<<end.timeSpec();
    ...
    if (!start.isValid() || !end.isValid() || end < start) {
        if (sch && sch->resource()) {
            kDebug(planDbg()) << sch->resource()->name() << sch->name() << "Available:" << sch->resource()->availableFrom() << sch->resource()->availableUntil();
        }

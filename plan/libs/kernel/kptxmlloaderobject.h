...
public:
    ...
    QString workVersion() const
    {
        return m_workversion;
    }
    void setWorkVersion(const QString &ver)
    {
        m_workversion = ver;
    }
    ...
    const KDateTime::Spec &projectSpec() const
    {
        return m_projectSpec;
    }
    void setProjectSpec(const KDateTime::Spec &spec)
    {
        m_projectSpec = spec;
    }
    ...
    {
        return QString("%1 seconds").arg((double)m_elapsed / 1000);
    }
    ...
    void addMsg(int sev, const QString &msg)
    {
        ...
        if (sev == Errors) {
            s = "ERROR";
        } else if (sev == Warnings) {
            s = "WARNING";
        } else if (sev == Diagnostics) {
            s = "Diagnostic";
        } else if (sev == Debug) {
            s = "Debug";
        } else {

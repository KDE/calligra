...
public:
    ...
    /// Set the time from when the resource is available to this project
    void setAvailableFrom(const DateTime &af)
    {
        m_availableFrom = af;
        changed();
    }
    ...
    /// Set the time when the resource is no longer available to this project
    void setAvailableUntil(const DateTime &au)
    {
        m_availableUntil = au;
        changed();
    }
    ...
    int numAppointments(long id = -1) const
    {
        return appointments(id).count();
    }
    /// Return the appointment at @p index for schedule @p id
    Appointment *appointmentAt(int index, long id = -1) const
    {
        return appointments(id).value(index);
    }
    int indexOf(Appointment *a, long id = -1) const
    {
        return appointments(id).indexOf(a);
    }
    ...
    {
        return cost.normalRate;
    }
    void setNormalRate(double rate)
    {
        cost.normalRate = rate;
        changed();
    }
    double overtimeRate() const
    {
        return cost.overtimeRate;
    }
    void setOvertimeRate(double rate)
    {
        cost.overtimeRate = rate;
        changed();
    }
    ...
    {
        m_currentSchedule = findSchedule(id);
    }
    ...
    bool isScheduled() const;
    QHash<long, Schedule *> schedules() const
    {
        return m_schedules;
    }
    /**
    ...
    int numExternalAppointments() const
    {
        return m_externalAppointments.count();
    }
    QList<Appointment *> externalAppointmentList() const
    {
        return m_externalAppointments.values();
    }
    ...
    QStringList requiredIds() const
    {
        return m_requiredIds;
    }
    ...
    const WorkInfoCache &workInfoCache() const
    {
        return m_workinfocache;
    }
...
public:
    ...
    */
    ...
    void setAllocatedDynaically(bool dyn)
    {
        m_dynamic = dyn;
    }
...
public:
    ...
    void unregister(const ResourceGroup *group)
    {
        if (group == m_group) {
            m_group = 0;
        }
    ...
    int count() const
    {
        return m_resourceRequests.count();
    }
    ResourceRequest *requestAt(int idx) const
    {
        return m_resourceRequests.value(idx);
    }
...
public:
    ...
    QList<ResourceGroupRequest *> requests() const
    {
        return m_requests;
    }

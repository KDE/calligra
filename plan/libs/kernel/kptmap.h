...
public:
    ...
    virtual bool contains(const QDate &date) const
    {
        return DateMapType::contains(date.toString(Qt::ISODate));
    }
    ...
    void insert(const QDate &date, int state = CalendarDay::NonWorking)
    {
        insert(date.toString(Qt::ISODate), state);
    }
    ...
    {
        return state(date.toString(Qt::ISODate));
    }
    ...
    void toggle(const QDate &date, int state = CalendarDay::NonWorking)
    {
        return toggle(date.toString(Qt::ISODate), state);
    }
...
public:
    ...
    {
        return IntMap::contains(week * 10000 + year);
    }
    bool contains(const QPair<int, int> &week)
    {
        return contains(week.first,  week.second);
    }

    void insert(int week, int year, int state = CalendarDay::NonWorking)
    {
        if (week < 1 || week > 53) {
            kError() << "Illegal week number: " << week << endl;
            return;
        }
        IntMap::insert(week * 10000 + year, state);
    }
    void insert(const QPair<int, int> &week, int state = CalendarDay::NonWorking)
    {
        insert(week.first, week.second, state);
    }

    void insert(WeekMap::iterator it, int state)
    {
        insert(week(it.key()), state);
    }

    void remove(const QPair<int, int> &week)
    {
        IntMap::remove(week.first * 10000 + week.second);
    }

    static QPair<int, int> week(int key)
    {
        return QPair<int, int>(key / 10000, key % 10000);
    }

    using IntMap::state;
    int state(const QPair<int, int> &week)
    {
        return IntMap::state(week.first * 10000 + week.second);
    }
    int state(int week, int year)
    {
        return state(QPair<int, int>(week, year));
    }

    void toggle(const QPair<int, int> &week, int state = CalendarDay::NonWorking)
    {
        if (week.first < 1 || week.first > 53) {
            kError() << "Illegal week number: " << week.first << endl;
            return;
        }

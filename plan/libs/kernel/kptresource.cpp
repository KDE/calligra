...
DateTimeInterval Resource::requiredAvailable(Schedule *node, const DateTime &start, const DateTime &end) const
{
    ...
    if (m_currentSchedule) {
        m_currentSchedule->logDebug(QString("Required available in interval: %1").arg(interval.toString()));
    }
#endif
...
#ifndef PLAN_NLOGDEBUG
        if (m_currentSchedule) {
            m_currentSchedule->logDebug(QString("Required available: no calendar, %1").arg(x.toString()));
        }
#endif
        return x;
    }
...
#ifndef PLAN_NLOGDEBUG
        if (m_currentSchedule) {
            m_currentSchedule->logDebug(QString("Required available: booked, %1").arg(i.toString()));
        }
#endif
        return i;
    }
...
#ifndef PLAN_NLOGDEBUG
    if (m_currentSchedule) {
        m_currentSchedule->logDebug(QString("Required first available in %1:  %2").arg(x.toString()).arg(i.toString()));
    }
#endif
    return i;
}
...
#ifndef PLAN_NLOGDEBUG
    if (m_currentSchedule) {
        QStringList lst; foreach (Resource * r, required) {
            lst << r->name();
        }
...
#ifndef PLAN_NLOGDEBUG
            if (m_currentSchedule) {
                m_currentSchedule->logDebug("The required resource '" + r->name() + "'is not available in interval:" + node->startTime.toString() + ',' + node->endTime.toString());
            }
#endif
            break;
        }
...
#if 0
    if (sch) {
        sch->logDebug(QString("Check effort in interval %1: %2, %3").arg(backward ? "backward" : "forward").arg(start.toString()).arg((backward ? start - duration : start + duration).toString()));
    }
#endif
    Duration e;
...
#ifndef PLAN_NLOGDEBUG
        if (sch) {
            sch->logDebug("Resource not available in interval:" + start.toString() + ',' + (start + duration).toString());
        }
#endif
    } else {
        foreach (Resource *r, required) {
...
#ifndef PLAN_NLOGDEBUG
                if (sch) {
                    sch->logDebug("The required resource '" + r->name() + "'is not available in interval:" + start.toString() + ',' + (start + duration).toString());
                }
#endif
                break;
            }
...
#ifndef PLAN_NLOGDEBUG
        if (sch && until < from) {
            sch->logDebug(" until < from: until=" + until.toString() + " from=" + from.toString());
        }
#endif
...
#ifndef PLAN_NLOGDEBUG
    if (sch) {
        sch->logDebug(QString("effort: %1 for %2 hours = %3").arg(start.toString()).arg(duration.toString(Duration::Format_HourFraction)).arg(e.toString(Duration::Format_HourFraction)));
    }
#endif
    return e;
}
...
#ifndef PLAN_NLOGDEBUG
    if (sch && t < lmt) {
        sch->logDebug("t < lmt: " + t.toString() + " < " + lmt.toString());
    }
#endif
    t = m_workinfocache.firstAvailableBefore(t, lmt, cal, sch);
//    t = cal->firstAvailableBefore(t, lmt, sch );
#ifndef PLAN_NLOGDEBUG
    if (sch && t.isValid() && t < lmt) {
        sch->logDebug(" t < lmt: t=" + t.toString() + " lmt=" + lmt.toString());
    }
#endif
    return t;
}
...
#ifndef PLAN_NLOGDEBUG
            if (ns) {
                ns->logDebug("Deviation match:" + logtime.toString() + " - " + end.toString() + " e=" + e.toString() + " (" + (_effort - e).toString() + ')');
            }
#endif
        }
    }

...
DateTime Task::calculatePredeccessors(const QList<Relation *> &list, int use)
{
    ...
    foreach (Relation *r, list) {
        ...
        case Relation::FinishFinish: {
            ...
            m_currentSchedule->logDebug("FinishFinish: get duration to calculate early finish");
#endif
...
DateTime Task::calculateSuccessors(const QList<Relation *> &list, int use)
{
    ...
    foreach (Relation *r, list) {
        ...
        case Relation::StartStart: {
            ...
            m_currentSchedule->setAllowOverbookingState(Schedule::OBS_Allow);
#ifndef PLAN_NLOGDEBUG
            m_currentSchedule->logDebug("StartStart: get duration to calculate late start");
#endif
...
#ifndef PLAN_NLOGDEBUG
            m_currentSchedule->logDebug("FinishFinish: get duration to calculate earliest start");
#endif
...
#ifndef PLAN_NLOGDEBUG
            m_currentSchedule->logDebug("StartStart: get duration to calculate late finish");
#endif
...
DateTime Task::scheduleFromEndTime(int use)
{
    ...
    m_requests.resetDynamicAllocations();
#ifndef PLAN_NLOGDEBUG
    cs->logDebug(QString("Finished schedule backward: %1 ms").arg(timer.elapsed()));
#endif
    return cs->startTime;
}
...
#ifndef PLAN_NLOGDEBUG
        if (sch) {
            sch->logDebug("Calculate length: estimate == 0");
        }
#else
        Q_UNUSED(sch)
#endif
...
#ifndef PLAN_NLOGDEBUG
        if (sch) {
            sch->logDebug("Calculate length: No calendar, return estimate " + duration.toString());
        }
#endif
        return duration;
    }
#ifndef PLAN_NLOGDEBUG
    if (sch) {
        sch->logDebug("Calculate length from: " + time.toString());
    }
#endif
...
#ifndef PLAN_NLOGDEBUG
        if (sch) {
            sch->logDebug("Days: duration " + logtime.toString() + " - " + end.toString() + " = " + l.toString() + " (" + (duration - l).toString() + ')');
        }
#endif
        logtime = start;
...
#ifndef PLAN_NLOGDEBUG
        if (sch) {
            sch->logDebug("Hours: duration " + logtime.toString() + " - " + end.toString() + " = " + l.toString() + " (" + (duration - l).toString() + ')');
        }
#endif
        logtime = start;
...
#ifndef PLAN_NLOGDEBUG
        if (sch) {
            sch->logDebug("Minutes: duration " + logtime.toString() + " - " + end.toString() + " = " + l.toString() + " (" + (duration - l).toString() + ')');
        }
#endif
        logtime = start;
...
#ifndef PLAN_NLOGDEBUG
        if (sch) {
            sch->logDebug("Seconds: duration " + logtime.toString() + " - " + end.toString() + " l " + l.toString() + " (" + (duration - l).toString() + ')');
        }
#endif
...
#ifndef PLAN_NLOGDEBUG
                if (sch) {
                    sch->logDebug("Got more than asked for, should not happen! Want: " + duration.toString(Duration::Format_Hour) + " got: " + l.toString(Duration::Format_Hour));
                }
#endif
                break;
            }
...
#ifndef PLAN_NLOGDEBUG
        if (sch) {
            sch->logDebug("Moved end to work: " + end.toString() + " -> " + t.toString());
        }
#endif
    }
...
#ifndef PLAN_NLOGDEBUG
        if (sch) {
            sch->logDebug("Calculated length: " + time.toString() + " - " + end.toString() + " = " + l.toString());
        }
#endif
    }
    return l;
...
#ifndef PLAN_NLOGDEBUG
        if (sch) {
            sch->logDebug(QString("workTimeAfter: %1 = %2").arg(dt.toString()).arg(t.toString()));
        }
#endif
    }

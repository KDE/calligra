...
public:
    ...
    QTime endTime() const
    {
        return first.addMSecs(second);
    }
    double hours() const
    {
        return (double)(second) / (1000. * 60. * 60.);
    }
    /// Returns true if this interval ends at midnight, and thus endTime() returns QTime(0,0,0)
    bool endsMidnight() const
    {
        return endTime() == QTime(0, 0, 0);
    }

    bool isValid() const
    {
        return first.isValid() && second > 0;
    }
    bool isNull() const
    {
        return first.isNull() || second < 0;
    }
...
public:
    ...
    QList<TimeInterval *> timeIntervals() const
    {
        return m_timeIntervals;
    }
    void addInterval(const QTime &t1, int length)
    {
        addInterval(new TimeInterval(t1, length));
    }
    /**
    ...
    void addInterval(TimeInterval interval)
    {
        addInterval(new TimeInterval(interval));
    }
    void clearIntervals()
    {
...
public:
    ...
    {
        QList<CalendarDay *> lst = m_weekdays.values();
        return lst;
    }
    /**
...
public:
    ...
    const Calendar &operator=(const Calendar &calendar)
    {
    ...
    Calendar *childAt(int index) const
    {
        return m_calendars.value(index);
    }
    ...
    int indexOf(const CalendarDay *day) const
    {
        return m_days.indexOf(const_cast<CalendarDay *>(day));
    }
    CalendarDay *dayAt(int index)
    {
        return m_days.value(index);
    }
    int numDays() const
    {
        return m_days.count();
    ...
    CalendarWeekdays *weekdays() const
    {
        return m_weekdays;
    }
    CalendarDay *weekday(int day) const
    {
        return m_weekdays->weekday(day);
    }
    int indexOfWeekday(const CalendarDay *day) const
    {
        return m_weekdays->indexOf(day);
    }
    const QList<CalendarDay *> weekdayList() const
    {
        return m_weekdays->weekdays();
    }
    int numWeekdays() const
    {
    ...
    {
        return findCalendar(m_id);
    }
    Calendar *findCalendar(const QString &id) const;
    bool removeId()
    {
        return removeId(m_id);
    }
...
public:
    ...
    double year() const
    {
        return m_year.toDouble(Duration::Unit_h);
    }
    ...
    {
        m_year = Duration((qint64)(hours * 60.0 * 60.0 * 1000.0));
    }
    ...
    double month() const
    {
        return m_month.toDouble(Duration::Unit_h);
    }
    ...
    {
        m_month = Duration((qint64)(hours * 60.0 * 60.0 * 1000.0));
    }
    ...
    double week() const
    {
        return m_week.toDouble(Duration::Unit_h);
    }
    ...
    {
        m_week = Duration((qint64)(hours * 60.0 * 60.0 * 1000.0));
    }
    ...
    double day() const
    {
        return m_day.toDouble(Duration::Unit_h);
    }
    ...
    {
        m_day = Duration(hours, Duration::Unit_h);
        changed();
    }

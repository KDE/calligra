...
public:
    ...
    {
        return m_resourceGroups.indexOf(resource);
    }
    ResourceGroup *resourceGroupAt(int pos) const
    {
        return m_resourceGroups.value(pos);
    }
    ...
    Calendar *defaultCalendar() const
    {
    ...
    Calendar *calendarAt(int index) const
    {
    ...
    virtual const QHash<QString, Node *> &nodeDict()
    {
        return nodeIdDict;
    }
    ...
    {
        return m_managers.indexOf(const_cast<ScheduleManager *>(sm));
    }
    ...
    {
        return m_config ? *m_config : emptyConfig;
    }
    ...
    const Task &taskDefaults() const
    {
        return config().taskDefaults();
    }

    /// Return locale. (Used for currency, everything else is from KGlobal::locale)
    KLocale *locale()
    {
        return const_cast<ConfigBase &>(config()).locale();
    }
    /// Return locale. (Used for currency, everything else is from KGlobal::locale)
    ...
    const QMap<QString, SchedulerPlugin *> &schedulerPlugins() const
    {

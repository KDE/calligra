...
{
    kDebug(planDbg()) << indent <<"Schedule[" << m_id <<"] '" << m_name <<"' type:" << typeToString() <<" (" << m_type <<")" << ( isDeleted() ?"   Deleted" :"" );
}
void NodeSchedule::printDebug( const QString& _indent )
{
    ...
    indent += "!  ";
    if ( m_parent == 0 )
        kDebug(planDbg()) << indent <<"No parent schedule!";
    if ( !notScheduled ) {
//FIXME: QT3 support
/*        if ( node() )
            kDebug(planDbg()) << indent <<"Node:" << node() ->name();
        else
            kDebug(planDbg()) << indent <<"No parent node!";*/
    }
    kDebug(planDbg()) << indent <<"Not scheduled=" << notScheduled;
    kDebug(planDbg()) << indent <<"Start time:" << startTime.toString();
    kDebug(planDbg()) << indent <<"End time:" << endTime.toString();
    kDebug(planDbg()) << indent <<"Duration:" << duration.seconds() << QByteArray(" secs" ) <<" (" << duration.toString() <<")";
    kDebug(planDbg()) << indent <<"Earliest start:" << earlyStart.toString();
    kDebug(planDbg()) << indent <<"Latest finish:" << lateFinish.toString();

    kDebug(planDbg()) << indent <<"schedulingError=" << schedulingError;
    Task *t = qobject_cast<Task*>( m_node );
    if ( t ) {
        kDebug(planDbg()) << indent <<"Effort not met="<<t->effortMetError();
    }
    kDebug(planDbg()) << indent <<"resourceError=" << resourceError;
    kDebug(planDbg()) << indent <<"resourceNotAvailable=" << resourceNotAvailable;
    kDebug(planDbg()) << indent <<"Resource overbooked=" << resourceOverbooked;
    if ( resourceOverbooked ) {
        kDebug(planDbg()) << indent <<"" << overbookedResources();
    }

    kDebug(planDbg()) << indent <<"inCriticalPath=" << inCriticalPath;
    kDebug(planDbg()) << indent;
    kDebug(planDbg()) << indent <<"workStartTime=" << workStartTime.toString();
    kDebug(planDbg()) << indent <<"workEndTime=" << workEndTime.toString();
    kDebug(planDbg()) << indent;
    kDebug(planDbg()) << indent <<"Appointments (Scheduling):" << m_appointments.count();
    foreach( Appointment *a, m_appointments ) {
        a->printDebug( indent + "  " );
    }
    kDebug(planDbg()) << indent <<"Appointments (Forward):" << m_forward.count();
    foreach( Appointment *a, m_forward ) {
        a->printDebug( indent + "  " );
    }
    kDebug(planDbg()) << indent <<"Appointments (Backward):" << m_backward.count();
    foreach( Appointment *a, m_backward ) {
        a->printDebug( indent + "  " );
    }
}
void ResourceSchedule::printDebug( const QString& _indent )
{
    ...
    if ( m_parent == 0 )
        kDebug(planDbg()) << indent <<"No parent schedule!";
    if ( resource() )
        kDebug(planDbg()) << indent <<"Resource:" << resource() ->name();
    else
        kDebug(planDbg()) << indent <<"No parent resource!";
    kDebug(planDbg()) << indent;
    kDebug(planDbg()) << indent <<"Appointments:" << m_appointments.count();
    kDebug(planDbg()) << indent;
    kDebug(planDbg()) << indent <<"Appointments (Scheduling):" << m_appointments.count();
    foreach( Appointment *a, m_appointments ) {
        a->printDebug( indent + "  " );
    ...
    kDebug(planDbg()) << indent;
    kDebug(planDbg()) << indent <<"Appointments (Total):";
...
//         kDebug(planDbg()) << indent <<"No parent node!";
    ...
    kDebug(planDbg())<< indent << "Scheduling log:"<<m_log.count();
    if ( ! m_log.isEmpty() ) {
        indent += "!-- ";
        QList<Schedule::Log>::ConstIterator it;
        for (it = m_log.constBegin(); it != m_log.constEnd(); ++it) {
            kDebug(planDbg())<< indent << logPhase( (*it).phase ) << (*it).node->name() << ((*it).resource? (*it).resource->name():"") << (*it).message;
        }
    }
}
#endif

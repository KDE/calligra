...
void DependencyScene::moveItem(DependencyNodeItem *item, const QList<Node *> &lst)
{
    ...
    if (newParent == m_project) {
        ...
        kDebug(planDependencyEditorDbg()) << newParent->name() << newParent->level();
    }
        ...
        if (focusItem()->type() == DependencyConnectorItem::Type) {
            ...
            if (from) {
                ...
                if (link) {
                    ...
                    kDebug(planDependencyEditorDbg()) << "No link";
                }
            }
        } else {
            kDebug(planDependencyEditorDbg()) << "Not connector type" << focusItem();
        }
    } else {
        kDebug(planDependencyEditorDbg()) << "No focusItem";
    }
...
void DependencyView::slotRelationAdded(Relation *rel)
{
    ...
    if (item == 0) {
        ...
        kDebug(planDependencyEditorDbg()) << "Relation already exists!";
    }
}
...
void DependencyView::slotRelationRemoved(Relation *rel)
{
    ...
    if (item) {
        ...
        kDebug(planDependencyEditorDbg()) << "Relation does not exist!";
    }
}
...
void DependencyView::slotNodeRemoved(Node *node)
{
    ...
    if (item) {
        ...
        kDebug(planDependencyEditorDbg()) << "Node does not exist!";
    }
    slotWbsCodeChanged();
}
...
void DependencyEditor::slotDeleteTask()
{
    ...
    foreach (Node * n, lst) {
        kDebug(planDependencyEditorDbg()) << n->name();
    }

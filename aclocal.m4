...
if test -z "$2"; then
  if test $kde_qtver = 2; then
    if test $kde_qtsubver -gt 0; then
      kde_qt_minversion="snapshot >= Qt 2.1"
    else
...
AC_DEFUN(AC_HAVE_XPM,
 ...
 if test "x$xpm_test" = xno; then
   ...
   [
    ...
    LDFLAGS="$X_LDFLAGS $USER_LDFLAGS $LDFLAGS $XPM_LDFLAGS $all_libraries -lXpm -lX11 -lXext $LIBZ $LIBSOCKET"
...
# This was formerly the freebsd* config, which breaks with freebsd4.0 which
# is also ELF; another case with freebsd4* would fix that
# The OpenBSD below might look strange, I don't know if one version of FreeBSD
# really used a.outs reported as OpenBSD by /usr/bin/file.
#  case "$host_os" in 
#    freebsdelf*)  
#    lt_cv_deplibs_check_method=pass_all
#    ;;

	...
	CanvasSection(x, y, width, height, canvas)
{
    ...
    propValues.first = "0";
    propValues.second << qApp->translate("CanvasKugarTemplate", "Page size");
    ...
    propValues.first = "0";
    propValues.second << qApp->translate("CanvasKugarTemplate", "Page orientation");
    propValues.second << "int_from_list";
    propValues.second << qApp->translate("CanvasKugarTemplate", "0 - Portrait");
    propValues.second << qApp->translate("CanvasKugarTemplate", "1 - Landscape");
    props["PageOrientation"] = propValues;
    propValues.second.clear();
    
    propValues.first = "0";
    propValues.second << qApp->translate("CanvasKugarTemplate", "Top margin");
    propValues.second << "int";
    props["TopMargin"] = propValues;
    propValues.second.clear();
    
    propValues.first = "0";
    propValues.second << qApp->translate("CanvasKugarTemplate", "Bottom margin");
    propValues.second << "int";
    props["BottomMargin"] = propValues;
    propValues.second.clear();
    
    propValues.first = "0";
    propValues.second << qApp->translate("CanvasKugarTemplate", "Left margin");
    propValues.second << "int";
    props["LeftMargin"] = propValues;
    propValues.second.clear();
    
    propValues.first = "0";
    propValues.second << qApp->translate("CanvasKugarTemplate", "Right margin");
    propValues.second << "int";
...
void CanvasKugarTemplate::arrangeSections(bool destructive)
{
    ...
    map<int, DetailBand>::reverse_iterator itr;
...
QString CanvasKugarTemplate::getXml()
{
    ...
    for (map<QString, pair<QString, QStringList> >::const_iterator it = props.begin();
        it != props.end(); it++ )
    {
	...
	CanvasDetailHeader **header, CanvasDetailFooter **footer)
{
    ...
    *footer = 0;
	...
	pageFooter = 0;
    for (map<int, DetailBand>::iterator i = details.begin(); i != details.end(); i++)
    {
	if (i->second.second == section)
	{
	    ...
	    *footer = i->second.first.second;
	    ...
	    detailsCount--;
	    detailsCount < 0 ? detailsCount = 0 : detailsCount=detailsCount;
	}
...
void CanvasReportHeader::draw(QPainter &painter)
{
    painter.drawText(rect(), AlignVCenter | AlignLeft,
		     qApp->translate("CanvasReportHeader", "Report Header"));
...
{
    painter.drawText(rect(), AlignVCenter | AlignLeft,
		     qApp->translate("CanvasReportFooter", "Report Footer"));
    CanvasBand::draw(painter);
}
...
{
    painter.drawText(rect(), AlignVCenter | AlignLeft,
		     qApp->translate("CanvasPageHeader", "Page Header"));
    CanvasBand::draw(painter);
}
...
{
    painter.drawText(rect(), AlignVCenter | AlignLeft,
		     qApp->translate("CanvasPageFooter", "Page Footer"));
    CanvasBand::draw(painter);
}
...
//		     qApp->translate("CanvasDetailHeader", "Detail Header"));
    CanvasBand::draw(painter);
}
...
void CanvasDetail::draw(QPainter &painter)
{
    ...
    painter.drawText(rect(), AlignVCenter | AlignLeft, str);
//		     qApp->translate("CanvasDetail", "Detail"));
    CanvasBand::draw(painter);
}
...
void CanvasDetailFooter::draw(QPainter &painter)
{
    ...
    painter.drawText(rect(), AlignVCenter | AlignLeft, str);
//		     qApp->translate("CanvasDetailFooter", "Detail Footer"));
    CanvasBand::draw(painter);
}

...
void RowHeaderItem::mousePressEvent(QGraphicsSceneMouseEvent * _ev)
{
    ...
    double dHeight = m_pView->zoomHandler()->unzoomItY(boundingRect().height());
    m_bResize = false;
    m_bSelection = false;
        ...
        if (m_frozen) {
            ...
            if (hiddenRows.count() > 0) {
                if (m_pView->selection()->isColumnSelected()) {
                    KMessageBox::error(0, i18n("Area is too large."));
                    return;
                }
...
void RowHeaderItem::equalizeRow(double resize)
{
    if (resize != 0.0) {
        ResizeRowManipulator* command = new ResizeRowManipulator();
...
void RowHeaderItem::mouseDoubleClickEvent(QGraphicsSceneMouseEvent*)
{
    if (!m_cellToolIsActive)
        return;
    register Sheet * const sheet = m_pView->activeSheet();
...
void RowHeaderItem::mouseMoveEvent(QGraphicsSceneMouseEvent * _ev)
{
    ...
    // The button is pressed and we are selecting ?
        ...
        else if (_ev->pos().y() > m_pCanvas->boundingRect().height()) {
            if (row < KS_rowMax) {
...
void RowHeaderItem::paintSizeIndicator(int mouseY)
{
    ...
    m_lSize->adjustSize();
    QRectF rcf = m_pCanvas->boundingRect();
...
void RowHeaderItem::updateRows(int from, int to)
{
    ...
    update(0, (int) y0, boundingRect().width(), (int)(y1 - y0));
}
...
//     ElapsedTime et( "Painting vertical header", ElapsedTime::PrintOnlyTime );
    ...
    // painting rectangle
    const QRectF paintRect = m_pView->zoomHandler()->viewToDocument(option->exposedRect);
    ...
    const double width = converter->viewToDocumentX(boundingRect().width() - 1);
...
void RowHeaderItem::focusOutEvent(QFocusEvent*)
{
    m_pView->disableAutoScroll();
    m_bMousePressed = false;
}

void RowHeaderItem::drawText(QPainter *painter, const QFont& font,
                         const QPointF& location, const QString& text) const
{
    ...
    forever {
        ...
        if (!line.isValid())
            break;
        line.setLineWidth(boundingRect().width() * scaleX);
    }
...
void ColumnHeaderItem::mousePressEvent(QGraphicsSceneMouseEvent * _ev)
{
    ...
    double ev_PosX;
    double dWidth = m_pView->zoomHandler()->unzoomItX(boundingRect().width());
...
void ColumnHeaderItem::equalizeColumn(double resize)
{
    if (resize != 0.0) {
        ResizeColumnManipulator* command = new ResizeColumnManipulator();
        command->setSheet(m_pView->activeSheet());
...
void ColumnHeaderItem::mouseDoubleClickEvent(QGraphicsSceneMouseEvent*)
{
    if (!m_cellToolIsActive)
        return;
    register Sheet * const sheet = m_pView->activeSheet();
...
void ColumnHeaderItem::mouseMoveEvent(QGraphicsSceneMouseEvent * _ev)
{
    ...
    // The button is pressed and we are selecting ?
        ...
        if (sheet->layoutDirection() == Qt::RightToLeft) {
            if (_ev->pos().x() < boundingRect().width() - m_pCanvas->boundingRect().width()) {
            ...
            } else if (_ev->pos().x() > boundingRect().width())
                m_pCanvas->horzScrollBar()->setValue((int)(m_pCanvas->horzScrollBar()->maximum() - (ev_PosX - dWidth + m_pView->zoomHandler()->unzoomItX(m_pCanvas->boundingRect().width()))));
        } else {
            if (_ev->pos().x() < 0)
                m_pCanvas->horzScrollBar()->setValue((int) ev_PosX);
            else if (_ev->pos().x() > m_pCanvas->boundingRect().width()) {
                if (col < KS_colMax) {
                    const ColumnFormat *cl = sheet->columnFormat(col + 1);
                    x = sheet->columnPosition(col + 1);
    ...
    // No button is pressed and the mouse is just moved
        ...
        if (sheet->layoutDirection() == Qt::RightToLeft) {
            ...
            while (x < m_pView->zoomHandler()->unzoomItY(boundingRect().width()) + m_pCanvas->xOffset() && tmpCol <= KS_colMax) {
                double w = sheet->columnFormat(tmpCol)->visibleWidth();
                //if col is hide and it's the first column
                //you mustn't resize it.
                if (ev_PosX >= x + w - unzoomedPixel &&
...
void ColumnHeaderItem::resizeEvent(QGraphicsSceneResizeEvent* _ev)
{
    ...
    if (sheet->layoutDirection() == Qt::RightToLeft && !QApplication::isRightToLeft()) {
        int dx = _ev->newSize().width() - _ev->oldSize().width();
        scroll(dx, 0);
...
void ColumnHeaderItem::paintSizeIndicator(int mouseX)
{
    ...
    if (sheet->layoutDirection() == Qt::RightToLeft)
        m_iResizePos = mouseX + m_pCanvas->boundingRect().width() - boundingRect().width();
    else
        m_iResizePos = mouseX;
    ...
    if (sheet->layoutDirection() == Qt::RightToLeft) {
        x = m_pCanvas->boundingRect().width() - x;
...
void ColumnHeaderItem::updateColumns(int from, int to)
{
    ...
    update((int) x0, 0, (int)(x1 - x0), boundingRect().height());
}
...
//     ElapsedTime et( "Painting horizontal header", ElapsedTime::PrintOnlyTime );
    ...
    if (sheet->layoutDirection() == Qt::RightToLeft) {
        ...
        x = sheet->leftColumn(int(m_pView->zoomHandler()->unzoomItX(boundingRect().width()) - paintRect.x() + m_pCanvas->xOffset()), xPos);
        //Align to the offset
        xPos = m_pView->zoomHandler()->unzoomItX(boundingRect().width()) - xPos + m_pCanvas->xOffset();
    } else {
        //Get the left column and the current x-position
    ...
    const KoViewConverter *converter = m_pCanvas->viewConverter();
    const double height = converter->viewToDocumentY(boundingRect().height() - 1);
...
void ColumnHeaderItem::focusOutEvent(QFocusEvent*)
{
    m_pView->disableAutoScroll();
    m_bMousePressed = false;
}

void ColumnHeaderItem::drawText(QPainter *painter, const QFont& font,

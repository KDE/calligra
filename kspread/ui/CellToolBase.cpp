...
QWidget* CellToolBase::createOptionWidget()
{
    ...
    d->optionWidget.setupUi(widget);

    d->locationComboBox = new LocationComboBox(widget, selection());
    d->locationComboBox->setMinimumWidth(100);
    d->optionWidget.gridLayout->addWidget(d->locationComboBox, 0, 4);

    d->optionWidget.userInput->setCellTool(this);
    d->optionWidget.formulaButton->setDefaultAction(action("insertFormula"));
...
bool CellToolBase::createEditor(bool clear, bool focus)
{
    ...
    if (!editor()) {
        ...
        xpos += m_canvas->viewConverter()->viewToDocumentX(m_canvas->canvasController()->canvasOffsetX());
        ...
        if (sheetDir == Qt::RightToLeft) {
            double dwidth = m_canvas->viewConverter()->viewToDocumentX(m_canvas->canvasWidget()->width());
            double w2 = qMax(w, min_w);
            xpos = dwidth - w2 - xpos;
        }

        double ypos = selection()->activeSheet()->rowPosition(selection()->marker().y());
        ypos += m_canvas->viewConverter()->viewToDocumentY(m_canvas->canvasController()->canvasOffsetY());
        ...
        if (!color.isValid())
            color = m_canvas->canvasWidget()->palette().text().color();
        ...
        editor()->setMinimumSize(QSize((int)m_canvas->viewConverter()->documentToViewX(min_w) - 1,
                                       (int)m_canvas->viewConverter()->documentToViewY(min_h) - 1));
        editor()->show();
...
void CellToolBase::focusEditorRequested()
{
    ...
    if (editor()) {
        if (selection()->lastEditorWithFocus() == Selection::EmbeddedEditor) {
            ...
            d->optionWidget.userInput->setFocus();
        }
    }
}
...
{
    QString text = d->optionWidget.userInput->toPlainText();
...
{
    CellFormatDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
}
...
void CellToolBase::createStyleFromCell()
{
    ...
    while (true) {
        styleName = KInputDialog::getText(i18n("Create Style From Cell"),
                                          i18n("Enter name:"), styleName, &ok, m_canvas->canvasWidget());
        ...
        if (styleName.length() < 1) {
            KMessageBox::sorry(m_canvas->canvasWidget(), i18n("The style name cannot be empty."));
            continue;
        }
...
void CellToolBase::font(const QString& font)
{
    ...
    if (editor()) {
        ...
        editor()->setEditorFont(style.font(), true, m_canvas->viewConverter());
        editor()->setFocus();
    } else {
        m_canvas->canvasWidget()->setFocus();
    }
...
{
    AngleDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
}
...
void CellToolBase::resizeColumn()
{
    if (selection()->isRowSelected())
        KMessageBox::error(m_canvas->canvasWidget(), i18n("Area is too large."));
    else {
        ResizeColumn dialog(m_canvas->canvasWidget(), selection());
        dialog.exec();
    }
}
...
{
    ShowColRow dialog(m_canvas->canvasWidget(), selection(), ShowColRow::Column);
    dialog.exec();
}
...
void CellToolBase::resizeRow()
{
    if (selection()->isColumnSelected())
        ...
        ResizeRow dialog(m_canvas->canvasWidget(), selection());
        dialog.exec();
    }
}
...
{
    ShowColRow dialog(m_canvas->canvasWidget(), selection(), ShowColRow::Row);
    dialog.exec();
}
...
{
    InsertDialog dialog(m_canvas->canvasWidget(), selection(), InsertDialog::Insert);
    dialog.exec();
}

void CellToolBase::deleteCells()
{
    InsertDialog dialog(m_canvas->canvasWidget(), selection(), InsertDialog::Remove);
    dialog.exec();
}
...
{
    CommentDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
}
...
{
    ConditionalDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
}
...
void CellToolBase::insertHyperlink()
{
    ...
    LinkDialog* dialog = new LinkDialog(m_canvas->canvasWidget(), selection());
...
{
    ValidityDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
}
...
{
    if (selection()->isSingular()) {
        KMessageBox::error(m_canvas->canvasWidget(), i18n("You must select multiple cells."));
        return;
    }

    SortDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
}
...
{
    selection()->emitAboutToModify();
    SeriesDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
}
...
void CellToolBase::insertFromDatabase()
{
    ...
    if (str.isEmpty()) {
        KMessageBox::error(m_canvas->canvasWidget(), i18n("No database drivers available.  To use this feature you need "
                           "to install the necessary Qt 3 database drivers."));
        return;
    }

    DatabaseDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
#endif
}
...
{
    ListDialog dialog(m_canvas->canvasWidget());
    dialog.exec();
}
...
{
    selection()->emitAboutToModify();
    ConsolidateDialog * dialog = new ConsolidateDialog(m_canvas->canvasWidget(), selection());
    dialog->show(); // dialog deletes itself later
}
...
void CellToolBase::goalSeek()
{
    ...
    GoalSeekDialog* dialog = new GoalSeekDialog(m_canvas->canvasWidget(), selection());
    dialog->show(); // dialog deletes itself later
}
...
void CellToolBase::subtotals()
{
    ...
    SubtotalDialog dialog(m_canvas->canvasWidget(), selection());
    if (dialog.exec()) {
...
{
    AddNamedAreaDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
}

void CellToolBase::namedAreaDialog()
{
    NamedAreaDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
}
...
void CellToolBase::formulaSelection(const QString& expression)
{
    ...
    createEditor();
    FormulaDialog* dialog = new FormulaDialog(m_canvas->canvasWidget(), selection(), editor(), expression);
    dialog->show(); // dialog deletes itself later
}
...
{
    SpecialPasteDialog dialog(m_canvas->canvasWidget(), selection());
    if (dialog.exec()) {
        selection()->emitModified();
    }
}
...
{
    FindDlg dialog(m_canvas->canvasWidget(), "Find", d->findOptions, d->findStrings);
...
{
    SearchDlg dialog(m_canvas->canvasWidget(), "Replace", d->findOptions, d->findStrings, d->replaceStrings);
    dialog.setHasSelection(!selection()->isSingular());
    dialog.setHasCursor(true);
...
void CellToolBase::slotHighlight(const QString &/*text*/, int /*matchingIndex*/, int /*matchedLength*/)
{
    ...
    globalRect.moveTopLeft(m_canvas->canvasWidget()->mapToGlobal(globalRect.topLeft()));
...
{
    GotoDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
}
...
void CellToolBase::spellCheck()
{
    ...
    if (selection()->activeSheet() == 0)
        return;

    d->spell.macroCmdSpellCheck = 0;
    d->spell.firstSpellSheet   = selection()->activeSheet();
    d->spell.currentSpellSheet = d->spell.firstSpellSheet;

    QRect selection = this->selection()->lastRange();

    // if nothing is selected, check every cell
    if (this->selection()->isSingular()) {
        d->spell.spellStartCellX = 0;
        d->spell.spellStartCellY = 0;
        d->spell.spellEndCellX  = 0;
        d->spell.spellEndCellY  = 0;
...
#warning TODO KDE4 port to sonnet
    ...
    if (doc()->getKSpellConfig()) {
        selection()->activeSheet()->doc()->getKSpellConfig()->setIgnoreList(doc()->spellListIgnoreAll());
        selection()->activeSheet()->doc()->getKSpellConfig()->setReplaceAllList(d->spell.replaceAll);

    }
    d->spell.kspell = new K3Spell(this, i18n("Spell Checking"), this,
                                  SLOT(spellCheckerReady()),
                                  selection()->activeSheet()->doc()->getKSpellConfig());
...
void CellToolBase::spellCheckerReplaceAll(const QString &orig, const QString & replacement)
{
...
void CellToolBase::spellCheckerIgnoreAll(const QString & word)
{
    selection()->activeSheet()->doc()->addIgnoreWordAll(word);
}


void CellToolBase::spellCheckerReady()
{
...
void CellToolBase::spellCleanup()
{
    useCursor(Qt::ArrowCursor);
...
bool CellToolBase::spellSwitchToOtherSheet()
{
// there is no other sheet
    if (selection()->activeSheet()->map()->count() == 1)
        return false;
...
// if the current sheet is the first one again, we are done.
    if (d->spell.currentSpellSheet == d->spell.firstSpellSheet) {
        selection()->emitVisibleSheetRequested(d->spell.firstSpellSheet);
        return false;
    }
    ...
    if (KMessageBox::questionYesNo(m_canvas->canvasWidget(),
                                   i18n("Do you want to check the spelling in the next sheet?"))
            != KMessageBox::Yes)
        return false;

    selection()->emitVisibleSheetRequested(d->spell.currentSpellSheet);

    return true;
}


void CellToolBase::spellCheckerMisspelling(const QString &,
        const QStringList &,
        unsigned int)
{
// scroll to the cell
    ...
    selection()->initialize(QPoint(d->spell.spellCurrCellX, d->spell.spellCurrCellY));
}


void CellToolBase::spellCheckerCorrected(const QString & old, const QString & corr, unsigned int pos)
{
    Cell cell;
    ...
    DataManipulator *command = new DataManipulator;
    command->setSheet(selection()->activeSheet());
    command->setText(i18n("Correct Misspelled Word"));
    command->setValue(Value(content));
...
// at once
    if (!d->spell.macroCmdSpellCheck) {
        ...
        mm->setSheet(selection()->activeSheet());
        mm->setRegisterUndo(false);
        d->spell.macroCmdSpellCheck = mm;
    }
    d->spell.macroCmdSpellCheck->add(command);
}

void CellToolBase::spellCheckerDone(const QString &)
{
...
void CellToolBase::spellCheckerFinished()
{
    useCursor(Qt::ArrowCursor);
    ...
    if (status == K3Spell::Error) {
        KMessageBox::sorry(m_canvas->canvasWidget(), i18n("ISpell could not be started.\n"
                           "Please make sure you have ISpell properly configured and in your PATH."));
        kspellNotConfigured = true;
    } else if (status == K3Spell::Crashed) {
        KMessageBox::sorry(m_canvas->canvasWidget(), i18n("ISpell seems to have crashed."));
    }
    ...
    if (kspellNotConfigured) {
        ...
        PreferenceDialog configDlg(m_canvas->canvasWidget(), 0);
...
{
    AutoFormatDialog dialog(m_canvas->canvasWidget(), selection());
    dialog.exec();
}
...
void CellToolBase::listChoosePopupMenu()
{
    ...
    if (selection()->activeSheet()->layoutDirection() == Qt::RightToLeft) {
        tx = m_canvas->canvasWidget()->width() - tx;
    }

    QPoint p((int)tx, (int)ty);
    QPoint p2 = m_canvas->canvasWidget()->mapToGlobal(p);

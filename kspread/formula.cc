...
/*
  ...
  - Ref means gets a value from a reference. Member variable 'index' will refers
    ...
    Example: expression "A2+B2" will be compiled as:
      Constants:
        #0: "A2"
        #1: "B2"
      Codes:
  ...
  The reduce rules are:
    ...
    Rule E: binary operator
      if syntax stack looks as:
        ...
        binary operator
        non-operator
      and if the precedence of the binary operator in the syntax stack
      is greater or equals to the precedence of token
      then reduce to:
        non-operator
  ...
  while( (state != Bad) && (state != Finish) && (i < ex.length()) )
  {
    ...
    case InIdentifier:
       ...
       else
       {
         ...
         if( ( tokenText.lower() == "true" ) || ( tokenText.lower() == "false" ) )
         {
           tokens.append( Token( Token::Boolean, tokenText, tokenStart ) );
           tokenStart = i;
           tokenText = "";
           state = Start;
         }
...
KSpreadValue Formula::convertToNumber( const KSpreadValue& value ) const
{
  ...
  switch( value.type() )
  {
    ...
    case KSpreadValue::String:
      ...
      {
        num2 = value.asString().toDouble( &ok );
...
KSpreadValue Formula::convertToString( const KSpreadValue& value ) const
{
  KSpreadValue result = KSpreadValue::errorVALUE();
  QString str;
...
KSpreadValue Formula::convertToBool( const KSpreadValue& value ) const
{
  KSpreadValue result = KSpreadValue::errorVALUE();
  QString str;
...
KSpreadValue Formula::eval() const
{
  ...
  for( unsigned pc = 0; pc < d->codes.count(); pc++ )
  {
    ...
    switch( opcode.type )
    {
      ...
      case Opcode::Concat:
        val1 = convertToString( stack.pop() );
        if( val1.isError() ) return KSpreadValue::errorVALUE();
        val2 = convertToString( stack.pop() );

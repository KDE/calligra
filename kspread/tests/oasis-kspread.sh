...
# We use the ksp format as a "dump" of the KPresenter data, to check if everything is correct
...
input=/mnt/devel/kofficetests/documents/native/kspread/test.ksp

test -f "$input" || { echo "No such file $input"; exit 1; }

# Load KPR
appid=`dcopstart kspread $input`
while `dcop $appid Document-0 isLoading` == "true"; do
    sleep 1;
done

# Save to KSP again (in case of changes in syntax etc.)
origfile=/tmp/oasisregtest-initial.ksp
dcop $appid Document-0 saveAs $origfile || exit 1
dcop $appid Document-0 waitSaveComplete || exit 1
test -f $origfile || exit 1

# Save to OASIS ODT
tmpoasisfile=/tmp/oasisregtest.ods
dcop $appid Document-0 setOutputMimeType "application/vnd.oasis.opendocument.spreadsheet" || exit 1
...
# Load resulting ODS, convert to KSP
tmpnativefile=/tmp/oasisregtest-final.ksp
...
dcop $appid Document-0 setOutputMimeType "application/x-kspread" || exit 1
dcop $appid Document-0 saveAs $tmpnativefile || exit 1
test -f $tmpnativefile || exit 1

# Compare initial and final KSP files

...
void StyleClusterTester::run()
{
  ...
  Doc* doc = new Doc();
  Map* workbook = doc->map();
  Sheet* sheet = workbook->addNewSheet();
  
  Style* defaultStyle = doc->styleManager()->defaultStyle();

  StyleCluster stylecluster(sheet);
  CHECK_STYLE(stylecluster.lookup(0,0), static_cast< const Style& > (*defaultStyle));
  CHECK_STYLE(stylecluster.lookup(1000,2000), static_cast< const Style& > (*defaultStyle));
  Style *style1 = new Style();
  style1->addRef();
  stylecluster.setStyle(1000,2000, style1);
  CHECK_STYLE(stylecluster.lookup(1000,2000), *style1); 
  CHECK_STYLE(stylecluster.lookup(1001,2000), static_cast< const Style& > (*defaultStyle));
  CHECK_STYLE(stylecluster.lookup(1001,2001), static_cast< const Style& > (*defaultStyle));
  CHECK_STYLE(stylecluster.lookup(1000,2001), static_cast< const Style& > (*defaultStyle));
  CHECK_STYLE(stylecluster.lookup(0,0), static_cast< const Style& > (*defaultStyle));
  ...
  CHECK_STYLE(stylecluster.lookup(0,2), static_cast< const Style& > (*defaultStyle));
...
//Quad tree should be totally empty now
  ...
  CHECK_STYLE(stylecluster.lookup(0,0), static_cast< const Style& > (*defaultStyle));
  CHECK_STYLE(stylecluster.lookup(1,1), static_cast< const Style& > (*defaultStyle));

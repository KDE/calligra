...
void TestValueParser::testTryParseBool_data(bool addCol)
{
    ...
    QTest::newRow("en TruE") << "C" << "TruE" << true << Value(true)<<""<<"";
    QTest::newRow("en fAlSe") << "C" << "fAlSe" << true << Value(false)<<""<<"";
    QTest::newRow("en xxtruexx") << "C" << "sann" << false << Value()<<""<<"";
...
void TestValueParser::testTryParseNumber_data(bool addCol)
{
    ...
    //    << true << complex<Number>(12.34e5, 4.2e2)<<""<<"";
    QTest::newRow("1,4 nl") << "da_DK" << "1,4" << true << Value(1.4)<<""<<"";
    QTest::newRow("1,400 nl") << "da_DK" << "1,400" << true << Value(1.4)<<""<<"";
}
...
void TestValueParser::testTryParseDate_data(bool addCol)
{
    ...
    QTest::newRow("LongDate wrong weekday") << "C" << "Friday, 01 January 2000" << true
        << Value(QDate(2000, 1, 1), m_calcsettings)<<"Qt does not handle wrong weekday"<<"";
    ...
    QTest::newRow("LongDate bad day 2") << "C" << "Saturday, 32 January 2000" << false
        << Value(QDate(), m_calcsettings)<<""<<"";
    QTest::newRow("LongDate short month") << "C" << "Tuesday, 1 feb 2000" << true
        << Value(QDate(2000, 2, 1), m_calcsettings)<<""<<"";
    QTest::newRow("LongDate bad month") << "C" << "Saturday 1, feburary 2000" << true
        << Value(QDate(), m_calcsettings)<<"Qt returns QDate(\"2000-01-01\")"<<"";

    QTest::newRow("IsoWeekFormat1") << "C" << "2004-W53-7" << true
        << Value(QDate(2005, 1, 2), m_calcsettings)<<"Weeknumbers not implemented"<<"";
    QTest::newRow("IsoWeekFormat2") << "C" << "2004-w53-7" << true
        << Value(QDate(2005, 1, 2), m_calcsettings)<<"Weeknumbers not implemented"<<"";
    QTest::newRow("IsoWeekFormat3") << "C" << "2005-W3-4" << true
        << Value(QDate(2005, 1, 20), m_calcsettings)<<"Weeknumbers not implemented"<<"";
    QTest::newRow("IsoWeekFormat4") << "C" << "2009-W01-1" << true
        << Value(QDate(2008, 12, 29), m_calcsettings)<<"Weeknumbers not implemented"<<"";
    ...
    QTest::newRow("IsoOrdinalFormat1") << "C" << "2004-1" << true
        << Value(QDate(2004, 1, 1), m_calcsettings)<<"Ordinal format not implemented"<<"";
    QTest::newRow("IsoOrdinalFormat2") << "C" << "2005-51" << true
        << Value(QDate(2005, 2, 20), m_calcsettings)<<"Ordinal format not implemented"<<"";
    QTest::newRow("IsoOrdinalFormat3") << "C" << "2006-151" << true
        << Value(QDate(2006, 5, 31), m_calcsettings)<<"Ordinal format not implemented"<<"";
    QTest::newRow("IsoOrdinalFormat4") << "C" << "2000-366" << true
        << Value(QDate(2000, 12, 31), m_calcsettings)<<"Ordinal format not implemented"<<"";
    ...
    QTest::newRow("ShortDate nl") << USE_LOCALE << "13.06.2005" << true
        << Value(QDate(2005, 6, 13), m_calcsettings)<<""<<"";
    QTest::newRow("ShortDate yy nl") << USE_LOCALE << "13.06.05" << true
        << Value(QDate(2005, 6, 13), m_calcsettings)<<""<<"";
    QTest::newRow("ShortDate nl m < 1") << USE_LOCALE << "13.0.05" << false
        << Value(QDate(), m_calcsettings)<<""<<"";
    QTest::newRow("ShortDate nl m > 12") << USE_LOCALE << "13.13.05" << false
        << Value(QDate(), m_calcsettings)<<""<<"";
...
void TestValueParser::testTryParseTime_data(bool addCol)
{
    ...
    QTest::newRow("24hr with seconds") << "C" << "13:14:15" << true << Value(Time(13, 14, 15))<<""<<"";
    QTest::newRow("0hr") << "C" << "0:14:15" << true << Value(Time(0, 14, 15))<<""<<"";
    QTest::newRow("0hr am") << "C" << "00:14:15 am" << true << Value(Time(0, 14, 15))<<""<<"";
    QTest::newRow("0hr pm") << "C" << "0:14:15 pm" << true << Value(Time(12, 14, 15))<<""<<"";
    QTest::newRow("0hr pM") << "C" << "0:14:15 pM" << true << Value(Time(12, 14, 15))<<"Cannot handle pM (only PM/pm)"<<"";
    QTest::newRow("24hr with neg seconds") << "C" << "13:14:-1" << true << Value(Time(12, 13, 59))<<"TODO: Negative values"<<"";
    QTest::newRow("24hr with large seconds") << "C" << "13:14:60" << true << Value(Time(13, 14, 60))<<"TODO: Too large values"<<"";
    QTest::newRow("24hr w/o seconds") << "C" << "13:14" << true << Value(Time(13, 14))<<""<<"";
    QTest::newRow("25hr duration") << "C" << "25:01:04" << true << Value(Time(25, 1, 4))<<""<<"";
    QTest::newRow("12hr") << "C" << "10:14:15" << true << Value(Time(10, 14, 15))<<""<<"";
    QTest::newRow("am with seconds") << "C" << "10:11:12 am" << true << Value(Time(10, 11, 12))<<""<<"";
    QTest::newRow("am w/o seconds") << "C" << "10:11 am" << true << Value(Time(10, 11))<<""<<"";
    QTest::newRow("pm with seconds") << "C" << "10:11:12 pm" << true << Value(Time(22, 11, 12))<<""<<"";
    QTest::newRow("pm w/o seconds") << "C" << "10:11 pm" << true << Value(Time(22, 11))<<""<<"";
    QTest::newRow("negative time") << "C" << "-1:30:10" << true << Value(Time(22, 29, 50))<<"TODO: Negative values"<<"";
    QTest::newRow("time with ms") << "C" << "2:3:4.5" << true << Value(Time(2, 3, 4, 500))<<""<<"";
    QTest::newRow("time with ms") << "C" << "02:03:04.5" << true << Value(Time(2, 3, 4, 500))<<""<<"";
    QTest::newRow("time with ms 2") << "C" << "2:3:4.999" << true << Value(Time(2, 3, 4, 999))<<""<<"";
    QTest::newRow("too low ms") << "C" << "2:3:4.-1" << true << Value(Time(2, 3, 4, -1))<<"TODO: Negative values"<<"";
    QTest::newRow("too high ms") << "C" << "2:3:4:1000" << true << Value(Time(2, 3, 4, 1000))<<"TODO: Too large values"<<"";
    QTest::newRow("24hr with neg minutes") << "C" << "13:-1:4" << true << Value(Time(13, -1, 4))<<"TODO: Negative values"<<"";
    QTest::newRow("24hr with large minutes") << "C" << "13:60:10" << true << Value(Time(13, 60, 10))<<"TODO: Too large values"<<"";

    QTest::newRow("24hr with seconds us") << "en_US" << "13:14:15" << false << Value()<<"Needs ap/pm"<<"";
    QTest::newRow("24hr w/o seconds us") << "en_US" << "13:14" << false << Value()<<"Needs am/pm"<<"";
    ...
    QTest::newRow("0hr us") << "en_US" << "0:14:15" << true << Value(Time(0, 14, 15))<<""<<"";
    QTest::newRow("0hr pm us") << "en_US" << "0:14:15 PM" << true << Value(Time(12, 14, 15))<<""<<"";
    QTest::newRow("0hr am us") << "en_US" << "0:14:15 AM" << true << Value(Time(0, 14, 15))<<""<<"";
    QTest::newRow("12hr us") << "en_US" << "10:14:15" << true << Value(Time(10, 14, 15))<<""<<"";
    QTest::newRow("am with seconds us") << "en_US" << "10:11:12 am" << true << Value(Time(10, 11, 12))<<""<<"";
    QTest::newRow("am w/o seconds us") << "en_US" << "10:11 am" << true << Value(Time(10, 11))<<""<<"";
    QTest::newRow("pm with seconds us") << "en_US" << "10:11:12 pm" << true << Value(Time(22, 11, 12))<<""<<"";
    QTest::newRow("pm w/o seconds us") << "en_US" << "10:11 pm" << true << Value(Time(22, 11))<<""<<"";
    QTest::newRow("time with ms us") << "en_US" << "2:3:4.5 am" << true << Value(Time(2, 3, 4, 500))<<""<<"";
    QTest::newRow("noon us") << "en_US" << "12:00 pm" << true << Value(Time(12, 0))<<""<<"";
    QTest::newRow("midnight us") << "en_US" << "12:00 am" << true << Value(Time(0, 0))<<""<<"";
    QTest::newRow("noon us") << "en_US" << "12:00pm" << true << Value(Time(12, 0))<<"TODO: Missing space before pm"<<"";
    QTest::newRow("midnight us") << "en_US" << "12:00am" << true << Value(Time(0, 0))<<"TODO: Missing space before am"<<"";
...
void TestValueParser::testTryParseDateTime_data()
{
    ...
    QTest::newRow("DT Iso short") << "C" << "2023-01-13T13:14:15" << true << QDate(2023, 1, 13) << QTime(13, 14, 15) <<"";
    QTest::newRow("DT Iso long") << "C" << "2023-01-13T13:14:15.999" << true << QDate(2023, 1, 13) << QTime(13, 14, 15, 999) <<"";

    QTest::newRow("DT short") << "C" << "13 Apr 2023 13:14:15" << true << QDate(2023, 4, 13) << QTime(13, 14, 15) << "";
    QTest::newRow("DT long") << "C" << "Thursday, 13 April 2023 13:14:15" << true << QDate(2023, 4, 13) << QTime(13, 14, 15) << "";

    QTest::newRow("DT short us") << "en_US" << "4/13/23 1:14 pm" << true << QDate(2023, 4, 13) << QTime(13, 14) << "";
    QTest::newRow("DT long us") << "en_US" << "Thursday, April 13, 2023 1:14:15 pm" << true << QDate(2023, 4, 13) << QTime(13, 14, 15) <<"";
}

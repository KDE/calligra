...
void TestValueConverter::testAsComplex_data()
{
    ...
    //    << true << complex<Number>(12.34e5, 4.2e2);
    QTest::newRow("string cz 1,4") << USE_LOCALE << Value("1,4")
        << true << complex<Number>(1.4, 0);
    QTest::newRow("string cz 1,400") << USE_LOCALE << Value("1,400")
        << true << complex<Number>(1.4, 0);
...
void TestValueConverter::testAsString_data()
{
    ...
    //    QTest::newRow("float datetime 2 us") << "en_US" << ValueWithFormat(-9.325, Value::fmt_DateTime) << Value("Wednesday, December 22, 1999 4:12:00 PM UTC");
...
//    QTest::newRow("float datetime 2 xx") << USE_LOCALE << ValueWithFormat(-9.325, Value::fmt_DateTime) << Value("onsdag 22. desember 1999 16:12:00 UTC");
    ...
    QTest::newRow("complex date 1 xx") << USE_LOCALE
        << ValueWithFormat(complex<Number>(0.5, 0), Value::fmt_Date)
        << Value("lørdag 1. januar 2000");
    QTest::newRow("complex date 2 xx") << USE_LOCALE
        << ValueWithFormat(complex<Number>(2000.324, 0), Value::fmt_Date)
        << Value("torsdag 23. juni 2005");
#endif
}

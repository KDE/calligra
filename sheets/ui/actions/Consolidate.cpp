...
void Consolidate::execute(Selection *selection, Sheet *, QWidget *canvasWidget)
{
    ...
    connect(m_dlg, &ConsolidateDialog::finished,
                this, &Consolidate::dialogClosed);
    connect(m_dlg, &ConsolidateDialog::applyChange,
                this, &Consolidate::applyChange);
    m_dlg->show();
}
    ...
    if (m_colHeaders) {
        for (Region & r : m_ranges) {
            ...
            for (int col = rect.left(); col <= rect.right(); ++col) {
                if ((col == rect.left()) && (m_rowHeaders)) continue;   // do not include the header
    ...
    if (m_rowHeaders) {
        for (Region & r : m_ranges) {
            ...
            for (int row = rect.top(); row <= rect.bottom(); ++row) {
                if ((row == rect.top()) && (m_colHeaders)) continue;   // do not include the header
...
{
    if (m_rowHeaders && (sourceCol == source.left())) return false;   // do not match headers
    if (m_colHeaders) {
    ...
    if (m_rowHeaders) sourceColIndex--;
    return (sourceColIndex == targetColIndex);
}

QString ConsolidateManipulator::rowHeader(int which) {
    if (!m_rowHeaders) return QString();
    if (m_colHeaders) which--;
    if (which < 0) return QString();
    if (which >= m_rowHeaderList.length()) return QString();  // out of range
    return m_rowHeaderList[which];
}

QString ConsolidateManipulator::colHeader(int which) {
    if (!m_colHeaders) return QString();
    if (m_rowHeaders) which--;
    if (which < 0) return QString();
    if (which >= m_colHeaderList.length()) return QString();  // out of range
...
Value ConsolidateManipulator::newValue(Element *element, int col, int row, bool *parse, Format::Type *) {
    ...
    for (Region & r : m_ranges) {
        ...
        for (int row = rect.top(); row <= rect.bottom(); ++row) {
            ...
            if (!matchingRow(rect, sheet, row, rowidx, targetHeader)) continue;
            for (int col = rect.left(); col <= rect.right(); ++col) {
                targetHeader = colHeader(colidx);
                if (!matchingCol(rect, sheet, col, colidx, targetHeader)) continue;

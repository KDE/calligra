 ...
 */
CALLIGRA_SHEETS_COMMON_EXPORT int decodeColumnLabelText(const QString &labelText);

/**
 * Call this function to decode the text of a row label to an integer,
 * e.g. B7 is translated to 7.
 */
CALLIGRA_SHEETS_COMMON_EXPORT int decodeRowLabelText(const QString &labelText);
 ...
 */
CALLIGRA_SHEETS_COMMON_EXPORT bool isCellReference(const QString &text, int startPos = 0);
 ...
 */
CALLIGRA_SHEETS_COMMON_EXPORT QString adjustFormulaReference(const QString& formula, int referencedRow, int referencedColumn, int thisRow, int thisColumn);

//Return true when it's a reference to cell from sheet.
CALLIGRA_SHEETS_COMMON_EXPORT bool localReferenceAnchor(const QString &_ref);

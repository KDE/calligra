...
{
    if (!m_symbols.isEmpty()) return;   // already loaded - do nothing
    ...
    for (auto l : allLocales) {
        ...
        if (!m_symbols.count(code)) m_symbols[code] = symbol;
    }
}
...
QString Currency::symbolToCode(const QString &symbol) {
    if (!symbol.size()) return QString();
    if (symbol == "$") return "USD";   // simple and common case
    QMap<QString, QString>::iterator it;
    for (it = m_symbols.begin(); it != m_symbols.end(); ++it) {
        QString key = it.key();
        if (key == symbol) return key;
        if (it.key() == symbol) return key;
    }

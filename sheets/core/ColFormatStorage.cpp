...
double ColFormatStorage::visibleWidth(int col, int *lastCol, int *firstCol) const
{
    if (isHiddenOrFiltered(col, lastCol, firstCol)) {
        ...
        if (lastCol)
            *lastCol = qMin(*lastCol, hLastCol);
        if (firstCol)
            *firstCol = qMax(*firstCol, hFirstCol);
        return width;
    }
}
...
bool ColFormatStorage::isHiddenOrFiltered(int col, int *lastCol, int *firstCol) const
{
    ...
    if (lastCol)
        *lastCol = qMin(hLastCol, fLastCol);
    if (firstCol)
        *firstCol = qMax(hFirstCol, fFirstCol);
    return v;
}
...
{
    return colWidth(col1) == colWidth(col2) && isHidden(col1) == isHidden(col2) && isFiltered(col1) == isFiltered(col2)
...
bool ColFormatStorage::isDefaultCol(int col, int *lastCol, int *firstCol) const
{
    ...
    if (lastCol)
        *lastCol = qMin(*lastCol, l);
    if (firstCol)
        *firstCol = qMax(*firstCol, f);

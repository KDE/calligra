...
bool CalendarDay::operator==(const CalendarDay &day) const {
    ...
    if (m_timeIntervals.count() != day.timeIntervals().count()) {
        //kDebug()<<m_timeIntervals.count()<<" !="<<day.timeIntervals().count();
        return false;
    }
...
DateTimeInterval Calendar::firstInterval( const KDateTime &start, const KDateTime &end, Schedule *sch) const
{
    ...
    if ( start.date() == end.date() ) {
        ...
        DateTimeInterval dti( DateTime( dt1.toLocalZone().dateTime() ), DateTime( dt1.addMSecs( res.second ).toLocalZone().dateTime() ) );
        //kDebug()<<"Got:"<<dti;
        return dti;
    }
    //kDebug()<<"tospec:"<<s.toString()<<" -"<<e.toString();
    ...
    for ( QDate date = start.date(); date <= end.date(); date = date.addDays(1) ) {
        ...
        if ( res.isValid() ) {
            ...
            DateTimeInterval dti( DateTime( dt1.toLocalZone().dateTime() ), dt1.addMSecs( res.second ).toLocalZone().dateTime() );

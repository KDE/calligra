...
int DependencyLinkItem::newChildColumn() const
{
    ...
    //kDebug()<<k_funcinfo<<"new col="<<col<<endl;
    return col;
}
...
{
    //kDebug()<<k_funcinfo<<rect()<<event->scenePos()<<endl;
...
{
    //kDebug()<<k_funcinfo<<text()<<": "<<rect<<endl;
    setRect( rect );
...
void DependencyNodeItem::moveToY( qreal y )
{
    ...
    r. moveTop( y );
    setRectangle( r );
    //kDebug()<<k_funcinfo<<text()<<" move to="<<y<<" new pos: "<<rect()<<endl;
    foreach ( DependencyLinkItem *i, m_parentrelations ) {
        i->createPath();
    }
    foreach ( DependencyLinkItem *i, m_childrelations ) {
...
void DependencyNodeItem::moveToX( qreal x )
{
    ...
    r. moveLeft( x );
    setRectangle( r );
    //kDebug()<<k_funcinfo<<m_text->toPlainText()<<" to="<<x<<" new pos: "<<rect()<<endl;
    foreach ( DependencyLinkItem *i, m_parentrelations ) {
        i->createPath();
    }
    foreach ( DependencyLinkItem *i, m_childrelations ) {
...
void DependencyNodeItem::setColumn()
{
    ...
    //kDebug()<<k_funcinfo<<column()<<", "<<text()<<endl;
}
...
void DependencyScene::drawBackground ( QPainter *painter, const QRectF &rect )
{
    ...
    for ( int r = first; r <= last; ++r ) {
        if ( r % 2 == 1 ) {
            ...
            //kDebug()<<k_funcinfo<<r<<": oy="<<oy<<" "<<rct<<endl;
        }
    }
}
...
void DependencyScene::setItemVisible( DependencyNodeItem *item, bool show )
{
    ...
    if ( row == -1 ) {
        kDebug()<<k_funcinfo<<"Unknown item!!"<<endl;
        return;
    }
    m_hiddenItems.clear();
...
DependencyNodeItem *DependencyScene::createItem( Node *node )
{
    ...
    if ( after ) {
        ...
        //kDebug()<<k_funcinfo<<"after="<<after->node()->name()<<" pos="<<i<<endl;
    }
...
void DependencyScene::mouseMoveEvent( QGraphicsSceneMouseEvent *mouseEvent )
{
    ...
    //kDebug()<<k_funcinfo<<mouseEvent->scenePos()<<", "<<mouseEvent->isAccepted()<<endl;
...
void DependencyView::slotConnectorClicked( DependencyConnectorItem *item )
{
    if ( itemScene()->fromItem() == 0 ) {
        ...
        //kDebug()<<k_funcinfo<<"emit makeConnection: "<<static_cast<DependencyNodeItem*>( item->parentItem() )->text()<<endl;
...
void DependencyView::slotRelationRemoved( Relation* rel )
{
    ...
    if ( item ) {
        ...
        delete item;
    } else kDebug()<<k_funcinfo<<"Relation does not exist!"<<endl;
}

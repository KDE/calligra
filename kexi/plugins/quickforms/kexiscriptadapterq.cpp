...
#include "kexiscriptadapterq.h"
    ...
    KexiScriptAdaptorQ::KexiScriptAdaptorQ() {
        setObjectName("Kexi");
    }
     ...
     */
    QWidget* KexiScriptAdaptorQ::windowWidget() const {
     ...
     */
    QWidget* KexiScriptAdaptorQ::viewWidget() const {
        return currentView();
    }
     ...
     * \endcode
     */
    QVariantList KexiScriptAdaptorQ::actions() {
        QVariantList list;
     ...
     */
    QObject* KexiScriptAdaptorQ::action(const QString& name) {
        foreach(QAction* action, mainWindow()->allActions()) {
     ...
     * is returned.
     */
    bool KexiScriptAdaptorQ::isConnected() {
 ...
 * \endcode
 */
QStringList KexiScriptAdaptorQ::items(const QString& className) {
    QStringList list;
    if (project()) {
        KexiPart::ItemList l;
 ...
 */
QString KexiScriptAdaptorQ::itemCaption(const QString& className, const QString& name) const {
 ...
 */
void KexiScriptAdaptorQ::setItemCaption(const QString& className, const QString& name, const QString& caption) {
 ...
 */
QString KexiScriptAdaptorQ::itemDescription(const QString& className, const QString& name) const {
    KexiPart::Item *item = partItem(partClass(className), name);
 ...
 */
void KexiScriptAdaptorQ::setItemDescription(const QString& className, const QString& name, const QString& description) {
    if (KexiPart::Item *item = partItem(partClass(className), name))
 ...
 * \endcode
 */
bool KexiScriptAdaptorQ::openItem(const QString& className, const QString& name, const QString& viewmode, QVariantMap args) {
 ...
 * \endcode
 */
bool KexiScriptAdaptorQ::closeItem(const QString& className, const QString& name) {
    if (KexiPart::Item *item = partItem(partClass(className), name))
 ...
 */
bool KexiScriptAdaptorQ::printItem(const QString& className, const QString& name, bool preview) {
    if (KexiPart::Item *item = partItem(partClass(className), name))
 ...
 */
bool KexiScriptAdaptorQ::executeItem(const QString& className, const QString& name, const QString& actionName) {
    if (KexiPart::Item *item = partItem(partClass(className), name))
 ...
 */
QString KexiScriptAdaptorQ::viewMode() const {
 ...
 */
QStringList KexiScriptAdaptorQ::viewModes() const {
    QStringList list;
    if (currentWindow()) {
 ...
 */
bool KexiScriptAdaptorQ::viewIsDirty() const {
    return currentView() ? currentView()->isDirty() : false;
}
    
    KexiMainWindowIface* KexiScriptAdaptorQ::mainWindow() const {
        return KexiMainWindowIface::global();
    }
    KexiProject* KexiScriptAdaptorQ::project() const {
        return mainWindow()->project();
    }
    KexiWindow* KexiScriptAdaptorQ::currentWindow() const {
        return mainWindow()->currentWindow();
    }
    KexiView* KexiScriptAdaptorQ::currentView() const {
        return currentWindow() ? currentWindow()->selectedView() : 0;
    }
    KexiPart::Item* KexiScriptAdaptorQ::partItem(const QString& className, const QString& name) const {
        return project() ? project()->itemForClass(partClass(className), name) : 0;
    }
    QString KexiScriptAdaptorQ::partClass(const QString& partClass) const {
        return partClass.contains('.') ? partClass : (QString::fromLatin1("org.kexi-project.")+partClass);
    }
    QString KexiScriptAdaptorQ::viewModeToString(Kexi::ViewMode mode, const QString& defaultViewMode) const {
        switch (mode) {
    ...
    Kexi::ViewMode KexiScriptAdaptorQ::stringToViewMode(const QString& mode, Kexi::ViewMode defaultViewMode) const {
        if (mode == "data")

...
void DoubleSpinBox::setValue(double v)
{
    if (m_property->option("unit", "").toString().isEmpty()) {
        ...
        KDoubleNumInput::setValue(KoUnit pageUnit(m_property->option("unit").toString()).toUserValue(v));
    }
}
...
double DoubleSpinBox::value() const
{
    if (m_property->option("unit", "").toString().isEmpty()) {
        ...
        return KoUnit pageUnit(m_property->option("unit").toString()).fromUserValue(KDoubleNumInput::value());
    }
}
...
//! @todo rounding using KLocale::formatNumber(const QString &numStr, bool round = true,int precision = 2)?
    ...
    if (prop->option("unit", "").toString().isEmpty()) {
        ...
        display = KGlobal::locale()->formatNumber(KoUnit pageUnit(prop->option("unit").toString()).toUserValue(prop->value().toDouble())) + " " + prop->option("unit").toString();
    }
    return display;
}

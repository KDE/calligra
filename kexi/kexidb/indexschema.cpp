...
{
	m_master_rels.setAutoDelete(true); //rels at the master-side are owned
}
...
IndexSchema::~IndexSchema()
{
	...
	*/
	...
	QPtrListIterator<Relationship> it(m_master_rels);
	for (;it.current();++it) {
		if (it.current()->detailsIndex()) {
			it.current()->detailsIndex()->detachRelationship(it.current());
		}
	}
	//ok, now m_master_rels will be just cleared automatically
}

...
{ 
	if (m_pkey) {
		m_pkey->setPrimary(false);
		m_pkey=0; //thats ok, the idx is still on indices list
	}
	
	if (!pkey) //clearing
		return;
	m_pkey = pkey; //todo
	m_pkey->setPrimary(true);
}
...
KexiDB::FieldList& TableSchema::addField(KexiDB::Field* field)
{
	...
	// this field auto-generate single-field unique index
	if (field->isPrimaryKey()) {
		IndexSchema *idx = new IndexSchema(this);
		idx->addField( field );
		idx->setPrimary(true);

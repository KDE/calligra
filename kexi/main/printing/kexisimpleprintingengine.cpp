	...
	while (delta > 0) {
		...
		if (realRect.height() > rect.height()) {
			...
			if (delta == text.length()) {
				realRect = getRealRect(fontMetrics, rect, alignFlags, text.left(cut)); //.stripWhiteSpace());
				break;
			}
			cut += delta / 2;
			delta /= 2;
...
#define MAX_PIXMAP_HEIGHT (m_mainLineSpacing * 5)
			...
			text = v.toString(); //.stripWhiteSpace();
...
//				painter.fontMetrics(), realRect, Qt::AlignTop|Qt::WordBreak,text);
				...
				QString aa( ((textOffsetForThisField > 0) ? text.mid(textOffsetForThisField) : text).left(cut).stripWhiteSpace() );
				kdDebug() << "-----" << aa << endl << "=====" << endl;
				kdDebug() << "-----text.mid(textOffsetForThisField):" << text.mid(textOffsetForThisField) << endl << "=====" << endl;
				kdDebug() << "-----text.mid(textOffsetForThisField).left(cut):" << text.mid(textOffsetForThisField).left(cut) << endl << "=====" << endl;
...
//				int(m_pageHeight - ((int)y - m_topMargin)),
				...
				((textOffsetForThisField > 0) ? text.mid(textOffsetForThisField) : text).left(cut-textOffsetForThisField).stripWhiteSpace());
		}
		if (m_settings->addTableBorders) {
			if (paint) {
				painter.setPen(Qt::darkGray);

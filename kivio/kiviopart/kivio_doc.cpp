...
bool KivioDoc::exportPage(KivioPage *pPage,const QString &fileName, ExportPageDialog *dlg)
{
  ...
  {
    if(dlg->fullPage()) {
      buffer.resize(zoom.zoomItX(pPage->getRectForAllStencils().w()) + dlg->border()*2,
        zoom.zoomItY(pPage->getRectForAllStencils().h()) + dlg->border()*2);
    } else {
      buffer.resize(zoom.zoomItX(pPage->getRectForAllSelectedStencils().w()) + dlg->border()*2,
        zoom.zoomItY(pPage->getRectForAllSelectedStencils().h()) + dlg->border()*2);
    }
  }
  else
  {

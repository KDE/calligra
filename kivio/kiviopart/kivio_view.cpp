...
void KivioView::addSpawnerToStackBar( KivioStencilSpawnerSet *pSpawner )
{
    if( !pSpawner )
    {
        debug("KivioView::addSpawnerToStackBar() - NULL pSpawner");
        return;
    }

    KivioIconView *pView = new KivioIconView();
...
void KivioView::groupStencils()
{
    ...
    debug("RECT: %3.2f %3.2f %3.2f %3.2f", r.x(), r.y(), r.w(), r.h() );
    m_pDoc->updateView(m_pActivePage);
}
...
void KivioView::slotSetStartArrow( int i )
{
    ...
    debug("Selected start arrow %d", i );
}

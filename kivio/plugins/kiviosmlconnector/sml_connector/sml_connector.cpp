...
{
	kdDebug() << "new sml stencil factory: " << endl;
}
...
void KivioSMLConnector::setStartPoint( double x, double y )
{
    ...
    // Set list end point
    m_shape->shapeData()->pointList()->first()->setX(x);
    m_shape->shapeData()->pointList()->first()->setY(y);
...
void KivioSMLConnector::setEndPoint( double x, double y )
{
    ...
    m_pEnd->disconnect();
    m_shape->shapeData()->pointList()->last()->setX(x);
    m_shape->shapeData()->pointList()->last()->setY(y);
}
